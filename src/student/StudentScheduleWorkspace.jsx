...
function ClubModal({ value, onApply, onClose }) {
  ...
  return <Dialog title="Shaxsiy mashg‘ulot" subtitle="Jadvalga eslatma — kursga a’zolik emas" onClose={onClose}>
...
export default function StudentScheduleWorkspace({ token, student, apiBase, onOpenTest, readOnly = false, compactTitle = false, mode = "student", initialGrade, language, schoolId, onFindCourses }) {
  ...
  return <section className={`student-schedule${readOnly ? " is-readonly" : ""}`} aria-label={teacher ? "O‘qituvchining shaxsiy dars rejasi" : "Haftalik dars jadvali"}>
    ...
    {data && <>
      ...
      <div className="student-schedule__clubs">{onFindCourses && <button type="button" className="schedule-main-button" style={{ marginBottom: 12 }} disabled={editing || loading || saving} title={editing ? "Avval jadvalni saqlang yoki tahrirni bekor qiling." : undefined} onClick={onFindCourses}>Kurs va to‘garak topish →</button>}<div className="student-schedule__clubs-head"><div><span>QO‘SHIMCHA RIVOJLANISH</span><h3>To‘garak va mashg‘ulotlar</h3><p>Shaxsiy mashg‘ulotlar va qo‘shimcha o‘qish.</p></div>{editable && <button type="button" disabled={loading || saving || clubs.length >= 20} onClick={() => openClub()}>+ Shaxsiy mashg‘ulot</button>}</div>{!clubs.length ? <p className="student-schedule__clubs-empty">Mashg‘ulot vaqtini kiriting yoki o‘qituvchining kursini toping.</p> : <div className="student-schedule__club-list">{clubs.map(item => <div key={item.id}><b aria-hidden="true">{CLUBS.find(club => club[0] === item.club_type)?.[1] || "✦"}</b><span><strong>{item.name}</strong><small>{item.day} · {item.time}{item.note ? ` · ${item.note}` : ""}</small></span>{editable && <><button type="button" aria-label={`${item.name}: tahrirlash`} disabled={loading || saving} onClick={() => openClub(item)}>✎</button><button type="button" aria-label={`${item.name}: o‘chirish`} disabled={loading || saving} onClick={() => { setNotice(""); setError(""); setDraft(old => { const next = old || clone(data); return { ...next, extracurriculars: next.extracurriculars.filter(club => club.id !== item.id) }; }); }}>×</button></>}</div>)}</div>}</div>
    </>}
