...
function Dialog({ title, subtitle, onClose, children }) {
  ...
  return createPortal(<div className="schedule-modal" onClick={event => { if (event.target === event.currentTarget) onClose(); }}><section className="schedule-modal__card" ref={card} role="dialog" aria-modal="true" aria-label={title}><header><div><small>{subtitle}</small><h3>{title}</h3></div><button type="button" onClick={onClose} aria-label={__kbUi("Yopish")}>×</button></header>{children}</section></div>, document.body);
}
function TopicModal({ base, queryParams, grade, lesson, dtsSubject, onApply, onClose }) {
  ...
  return <Dialog title={__kbUi("Darsni moslash")} subtitle={__kbUi(`${grade || ""}-sinf · ${lesson.subject}`)} onClose={onClose}>
    <p className="schedule-modal__hint">{__kbUi("Mavzu shu sinf va fan uchun bazadagi DTS ro‘yxatidan olinadi. O‘zgarish shaxsiy jadvalingizga tegishli.")}</p>
    <label className="schedule-field">{__kbUi("Dars turi")}<select value={choice.activity_type || "lesson"} onChange={event => setChoice(old => ({ ...old, activity_type: event.target.value }))}>{TYPES.map(([key, label]) => <option key={key} value={key}>{__kbUi(label)}</option>)}</select></label>
    <div className="schedule-topic-search"><label className="schedule-field">{__kbUi("Chorak")}<select value={quarter} onChange={event => setQuarter(event.target.value)}><option value="">{__kbUi("Barcha choraklar")}</option>{[1, 2, 3, 4].map(item => <option key={item} value={item}>{__kbUi(item)}{__kbUi("-chorak")}</option>)}</select></label><label className="schedule-field">{__kbUi("DTS mavzusini topish")}<input value={query} onChange={event => setQuery(event.target.value)} placeholder={__kbUi("Mavzu yoki bob nomi")} /></label></div>
    <div className="schedule-topic-selected"><small>{__kbUi("Tanlangan mavzu")}</small><strong>{choice.topic || __kbUi("Mavzu tanlanmagan")}</strong>{choice.topic_code && <button type="button" onClick={() => setChoice(old => ({ ...old, topic: "", topic_code: "" }))}>{__kbUi("Tanlovni tozalash")}</button>}</div>
    {status === "loading" ? <p className="schedule-topic-state" role="status">{__kbUi("DTS mavzulari yuklanmoqda…")}</p> : status === "error" ? <div className="schedule-topic-state" role="alert"><p>{__kbUi(error)}</p><button type="button" onClick={() => setRetry(old => old + 1)}>{__kbUi("Qayta urinish")}</button></div> : !shown.length ? <p className="schedule-topic-state">{__kbUi("Bu tanlov bo‘yicha bazada DTS mavzusi topilmadi. Boshqa chorak yoki qidiruvni tanlang.")}</p> : <div className="schedule-topic-list">{shown.map(item => <button type="button" key={item.topic_code} className={choice.topic_code === item.topic_code ? "active" : ""} aria-pressed={choice.topic_code === item.topic_code} onClick={() => setChoice(old => ({ ...old, topic_code: item.topic_code, topic: item.nomi }))}><b>{item.nomi}</b><small>{__kbUi([item.chorak ? `${item.chorak}-chorak` : "", item.bob_nomi, Number(item.test_count) > 0 ? `${item.test_count} test` : ""].filter(Boolean).join(" · "))}</small></button>)}</div>}
    <div className="schedule-modal__footer"><button type="button" onClick={onClose}>{__kbUi("Bekor qilish")}</button><button type="button" className="schedule-main-button" onClick={() => { onApply({ activity_type: choice.activity_type, topic: choice.topic, topic_code: choice.topic_code }); onClose(); }}>{__kbUi("Tanlovni qo‘llash")}</button></div>
  </Dialog>;
}
function MoveModal({ schedule, lessonTimes, from, onApply, onClose }) {
  ...
  return <Dialog title={__kbUi("Dars o‘rnini almashtirish")} subtitle={subject} onClose={onClose}>
    <p className="schedule-modal__hint">{__kbUi("Band o‘rinni tanlasangiz, ikki dars joy almashadi. Kun oxiriga ko‘chirsangiz, haftalik fan soatlari saqlanadi.")}</p>
    <label className="schedule-field">{__kbUi("Qaysi kunga?")}<select value={day} onChange={event => { setDay(Number(event.target.value)); setSlot(""); }}>{schedule.map((item, index) => <option key={item.day} value={index}>{item.day}{item.is_study_day === false ? __kbUi(" · dam olish kuni") : __kbUi("")}</option>)}</select></label>
    <div className="schedule-move-options">{lessons.map((lesson, index) => <button type="button" key={lesson.id || index} disabled={isLockedLesson(lesson, day, index) || (day === from.day && index === from.index)} className={slot === String(index) ? "active" : ""} onClick={() => setSlot(String(index))}><b>{index + 1}</b><span>{lesson.subject}<small>{lesson.time}</small></span></button>)}{(lessons.length < 8 || day === from.day) && <button type="button" className={slot === String(lessons.length) ? "active" : ""} onClick={() => setSlot(String(lessons.length))}>{__kbUi("+ Kun oxiriga ko‘chirish")}</button>}</div>
    {error && <p className="schedule-inline-error" role="alert">{__kbUi(error)}</p>}
    <div className="schedule-modal__footer"><button type="button" onClick={onClose}>{__kbUi("Bekor qilish")}</button><button type="button" className="schedule-main-button" disabled={slot === ""} onClick={() => { try { onApply(moveLesson(schedule, from, { day, index: Number(slot) }, lessonTimes)); onClose(); } catch (err) { setError(err.message); } }}>{__kbUi("Joylashtirish")}</button></div>
  </Dialog>;
}
function ClubModal({ value, onApply, onClose }) {
  ...
  return <Dialog title={__kbUi("Shaxsiy mashg‘ulot")} subtitle={__kbUi("Jadvalga eslatma — kursga a’zolik emas")} onClose={onClose}>
    <form onSubmit={event => { event.preventDefault(); if (!form.name.trim() || !/^\d{2}:\d{2}$/.test(form.time)) { setError("To‘garak nomi va vaqtini kiriting."); return; } onApply({ ...form, name: form.name.trim(), note: form.note.trim() }); onClose(); }}>
      <div className="schedule-club-fields"><label className="schedule-field">{__kbUi("Yo‘nalish")}<select value={form.club_type} onChange={event => { const key = event.target.value; setForm(old => ({ ...old, club_type: key, name: key === "other" ? old.name : CLUBS.find(item => item[0] === key)[2] })); }}>{CLUBS.map(([key, icon, name]) => <option key={key} value={key}>{__kbUi(icon)} {__kbUi(name)}</option>)}</select></label><label className="schedule-field">{__kbUi("Nomi")}<input required maxLength={100} value={form.name} onChange={event => setForm(old => ({ ...old, name: event.target.value }))} /></label><label className="schedule-field">{__kbUi("Kun")}<select value={form.day} onChange={event => setForm(old => ({ ...old, day: event.target.value }))}>{DAYS.map(day => <option value={(day)} key={day}>{__kbUi(day)}</option>)}</select></label><label className="schedule-field">{__kbUi("Vaqt")}<input required type="time" value={form.time} onChange={event => setForm(old => ({ ...old, time: event.target.value }))} /></label><label className="schedule-field schedule-field--wide">{__kbUi("Izoh")}<input maxLength={160} value={form.note} onChange={event => setForm(old => ({ ...old, note: event.target.value }))} placeholder={__kbUi("Manzil yoki ustoz")} /></label></div>
      {error && <p className="schedule-inline-error" role="alert">{__kbUi(error)}</p>}<div className="schedule-modal__footer"><button type="button" onClick={onClose}>{__kbUi("Bekor qilish")}</button><button type="submit" className="schedule-main-button">{__kbUi("Jadvalga qo‘shish")}</button></div>
    </form>
  </Dialog>;
}
function Lesson({ lesson, grade, editing, locked, onTopic, onMove, onTest }) {
  useKbInterfaceLocale();
  return <div className={`student-schedule__lesson${locked ? " is-locked" : ""}`}><div className="student-schedule__lesson-head"><b>{lesson.order}</b><time>{lesson.time}</time>{locked && <span title={__kbUi("Dushanba kuni birinchi dars")}>⌑</span>}</div><strong>{lesson.subject}</strong><p>{lesson.topic || (lesson.activity_type === "review" ? __kbUi("O‘tilganlarni takrorlash") : lesson.activity_type === "control" ? __kbUi("Nazorat mashg‘uloti") : __kbUi("DTS mavzusini tanlash mumkin"))}</p><small className={`student-schedule__type type-${lesson.activity_type || "lesson"}`}>{__kbUi(typeLabel(lesson.activity_type))}</small>{editing ? <div className="student-schedule__lesson-actions"><button type="button" onClick={onTopic}>{__kbUi("Mavzu / tur")}</button><button type="button" disabled={locked} onClick={onMove}>{__kbUi("⇄ Joyi")}</button></div> : lesson.topic_code && onTest ? <button type="button" className="student-schedule__practice" onClick={() => onTest(lessonTarget(lesson, grade))}>{__kbUi("Shu mavzudan test →")}</button> : null}</div>;
}

export default function StudentScheduleWorkspace({ token, student, apiBase, onOpenTest, readOnly = false, compactTitle = false, mode = "student", initialGrade, language, schoolId, onFindCourses }) {
  useKbInterfaceLocale();
  ...
  return <section className={`student-schedule${readOnly ? " is-readonly" : ""}`} aria-label={teacher ? __kbUi("O‘qituvchining shaxsiy dars rejasi") : __kbUi("Haftalik dars jadvali")}>
    <div className="student-schedule__hero"><div><span className="student-schedule__eyebrow">{readOnly ? __kbUi("FARZANDINGIZNING HAFTASI") : teacher ? __kbUi("SHAXSIY DARSGA TAYYORGARLIK") : __kbUi("BUGUNGI TA’LIM MAYDONI")}</span><h2>{teacher ? __kbUi("Haftalik dars rejam") : compactTitle ? __kbUi("Haftalik dars jadvali") : __kbUi(`Salom, ${student?.full_name || "o‘quvchi"}!`)}</h2><p>{grade ? __kbUi(`${grade}-sinf · `) : __kbUi("")}{data ? sourceText : __kbUi("Sinfga mos fanlar va DTS mavzulari")}</p></div>{!readOnly && documentData && <div className="student-schedule__actions">{editing ? <><span className="student-schedule__unsaved">{__kbUi("Saqlanmagan o‘zgarishlar")}</span><button type="button" className="schedule-secondary" onClick={cancelEditing} disabled={loading || saving}>{__kbUi("Bekor qilish")}</button><button type="button" onClick={save} disabled={loading || saving}>{saving ? __kbUi("Saqlanmoqda…") : __kbUi("Saqlash")}</button></> : <button type="button" onClick={beginEditing} disabled={!editable || loading}>{__kbUi("Jadvalni moslash")}</button>}</div>}</div>
    <div className="student-schedule__week-nav"><div className="student-schedule__week-buttons"><button type="button" aria-label={__kbUi("Oldingi hafta")} disabled={editing || loading || saving} onClick={() => setWeek(old => shiftWeek(old, -1))}>←</button><strong>{__kbUi(dateLabel(data?.week_start || week))} — {__kbUi(dateLabel(data?.week_end || shiftWeek(week, 1)))}</strong><button type="button" aria-label={__kbUi("Keyingi hafta")} disabled={editing || loading || saving} onClick={() => setWeek(old => shiftWeek(old, 1))}>→</button><button type="button" disabled={editing || loading || saving || week === mondayISO()} onClick={() => setWeek(mondayISO())}>{__kbUi("Joriy hafta")}</button></div>{teacher && <div className="student-schedule__teacher-select"><label>{__kbUi("Sinf")}<select aria-label={__kbUi("Reja uchun sinf")} value={selectedGrade || ""} disabled={editing || loading || saving} onChange={event => setSelectedGrade(Number(event.target.value))}><option value="" disabled>{__kbUi("Sinfni tanlang")}</option>{(data?.available_grades || Array.from({ length: 11 }, (_, index) => index + 1)).map(item => <option key={item} value={item}>{item}{__kbUi("-sinf")}</option>)}</select></label><label>{__kbUi("Ta’lim tili")}<select value={selectedLanguage || "uz"} disabled={editing || loading || saving} onChange={event => setSelectedLanguage(event.target.value)}><option value="uz">{__kbUi("O‘zbek")}</option><option value="ru">{__kbUi("Rus")}</option><option value="en">{__kbUi("Ingliz")}</option></select></label></div>}</div>
    {loading && <p className="student-schedule__notice" role="status">{editing ? __kbUi("Fanlar va mavzular qayta taqsimlanmoqda…") : __kbUi("Sinf o‘quv rejasi va hafta taqvimi yuklanmoqda…")}</p>}
    {error && <div className="student-schedule__error" role="alert"><span>{__kbUi(error)}</span>{!editing && <button type="button" onClick={() => loadSchedule()} disabled={loading}>{__kbUi("Qayta urinish")}</button>}{editing && <small>{__kbUi("Saqlash tasdiqlanmadi. Kerak bo‘lsa «Bekor qilish»dan keyin haftani qayta yuklang.")}</small>}</div>}
    {notice && <p className="student-schedule__notice" role="status">{__kbUi(notice)}</p>}
    {data && <>
      <div className="student-schedule__summary"><span><b>{total}</b>{__kbUi(" haftalik reja soati")}</span><span><b>{Object.keys(counts).length}</b>{__kbUi(" fan")}</span><span><b>{actualHours}</b>{__kbUi(" shu hafta dars")}</span><span>{quarters.length ? __kbUi(`${quarters.map(item => `${item}-chorak`).join(" / ")}`) : __kbUi("Chorak taqvimi belgilanmagan")}</span></div>
      {!!data.warnings?.length && <details className="student-schedule__warnings"><summary>{__kbUi("Jadval uchun ")}{data.warnings.length}{__kbUi(" ta izoh")}</summary><ul>{data.warnings.map((warning, index) => <li key={index}>{typeof warning === "string" ? __kbUi(warning) : warning.message || warning.detail || __kbUi("Admin taqvim sozlamalarini tekshiring.")}</li>)}</ul></details>}
      {editing && <div className="student-schedule__edit-help"><p><b>{__kbUi("Bu sizning shaxsiy rejangiz.")}</b>{__kbUi(" «Joyi» bilan darslarni almashtiring, «Mavzu / tur» orqali DTS mavzusini yoki takrorlash va nazoratni tanlang. Fanlarning haftalik soati saqlanadi.")}</p><div className="student-schedule__regenerate"><span>{__kbUi("Qayta tuzish shaxsiy joylashuv va mavzularni admin rejasidan yangilaydi.")}</span><button type="button" disabled={loading || saving} onClick={() => loadSchedule({ preview: true })}>{__kbUi("↻ Avto qayta tuzish")}</button><label>{__kbUi("Smena")}<select value={data.shift || 1} disabled={loading || saving} onChange={event => loadSchedule({ preview: true, shift: Number(event.target.value) })}><option value={1}>{__kbUi("1-smena")}</option><option value={2}>{__kbUi("2-smena")}</option></select></label>{canChooseDays && <label>{__kbUi("O‘qish haftasi")}<select value={data.study_days || schedule.length} disabled={loading || saving} onChange={event => loadSchedule({ preview: true, studyDays: Number(event.target.value) })}><option value={5}>{__kbUi("5 kun")}</option><option value={6}>{__kbUi("6 kun")}</option></select></label>}</div></div>}
      {editing && Object.entries(data.subject_options || {}).some(([, options]) => options?.length) && <div className="student-schedule__subject-mappings"><p>{__kbUi("Chet tili mavzularini qaysi fandan olish kerak? Tanlov o‘zgarsa, shaxsiy hafta qayta tuziladi.")}</p>{Object.entries(data.subject_options).filter(([, options]) => options?.length).map(([subject, options]) => <label key={subject}>{subject}<select value={data.subject_mappings?.[subject] || ""} disabled={loading || saving} onChange={event => loadSchedule({ preview: true, subjectMappings: { ...(data.subject_mappings || {}), [subject]: event.target.value } })}><option value="" disabled>{__kbUi("DTS fanini tanlang")}</option>{options.map(option => <option key={option} value={option}>{option}</option>)}</select></label>)}</div>}
      <div className="student-schedule__strip-heading"><p>{__kbUi("Barcha kunlar bir qatorda. Yon tomonga suring.")}</p><nav aria-label={__kbUi("Hafta kuniga o‘tish")}>{schedule.map((day, index) => <button type="button" key={day.date || day.day} className={day.date === todayISO ? "is-today" : ""} onClick={() => scrollDay(index)}>{__kbUi(day.day.slice(0, 3))}</button>)}</nav></div>
      <div className="student-schedule__grid" ref={weekStrip} style={{ "--schedule-days": Math.max(1, schedule.length) }} role="region" aria-label={__kbUi("Butun hafta jadvali; yon tomonga aylantirish mumkin")} tabIndex={0}>{schedule.map((day, dayIndex) => <article key={day.date || day.day} className={`${day.date === todayISO ? "is-today " : ""}${day.is_study_day === false ? "is-rest-day" : ""}`}><header><div><h3>{day.day}</h3><span>{__kbUi(dateLabel(day.date))}{day.date === todayISO ? __kbUi(" · Bugun") : __kbUi("")}</span></div><b>{day.lessons.length}{__kbUi(" dars")}</b></header>{day.is_study_day === false && <p className="student-schedule__rest-label">{day.reason || __kbUi("Dam olish kuni")}<small>{__kbUi("Quyida haftalik andozadagi fanlar.")}</small></p>}{day.lessons.length ? day.lessons.map((lesson, lessonIndex) => <Lesson key={lesson.id || `${dayIndex}-${lessonIndex}`} lesson={lesson} grade={grade} locked={isLockedLesson(lesson, dayIndex, lessonIndex)} editing={editing && !loading && !saving} onTopic={() => { if (editable) setTopicModal({ dayIndex, lessonIndex, lesson }); }} onMove={() => { if (editable) setMoveModal({ day: dayIndex, index: lessonIndex }); }} onTest={!editing && day.is_study_day !== false ? onOpenTest : null} />) : <p className="student-schedule__empty-day">{__kbUi("Bu kunga dars belgilanmagan.")}</p>}</article>)}</div>
      {!!curriculum.length && <details className="student-schedule__curriculum"><summary>{__kbUi("Fanlar va haftalik soatlarni tekshirish")}</summary><div className="student-schedule__table-wrap"><table><thead><tr><th>{__kbUi("Fan")}</th><th>{__kbUi("Admin reja soati")}</th><th>{__kbUi("Shu hafta andozasi")}</th><th>{__kbUi("Joylashtirildi")}</th></tr></thead><tbody>{curriculum.map((item, index) => <tr key={`${item.subject}-${index}`}><td>{item.subject}</td><td>{item.weekly_hours}</td><td>{item.week_hours ?? __kbUi("—")}</td><td>{counts[normalized(item.subject)] || 0}</td></tr>)}</tbody></table></div><p>{__kbUi("Kasrli soatlar haftalar bo‘yicha navbatlanishi mumkin. Bayram va ta’til kunlari admin taqvimiga ko‘ra alohida belgilanadi.")}</p></details>}
      <div className="student-schedule__clubs">{onFindCourses && <button type="button" className="schedule-main-button" style={{ marginBottom: 12 }} disabled={editing || loading || saving} title={editing ? __kbUi("Avval jadvalni saqlang yoki tahrirni bekor qiling.") : undefined} onClick={onFindCourses}>{__kbUi("Kurs va to‘garak topish →")}</button>}<div className="student-schedule__clubs-head"><div><span>{__kbUi("QO‘SHIMCHA RIVOJLANISH")}</span><h3>{__kbUi("To‘garak va mashg‘ulotlar")}</h3><p>{__kbUi("Shaxsiy mashg‘ulotlar va qo‘shimcha o‘qish.")}</p></div>{editable && <button type="button" disabled={loading || saving || clubs.length >= 20} onClick={() => openClub()}>{__kbUi("+ Shaxsiy mashg‘ulot")}</button>}</div>{!clubs.length ? <p className="student-schedule__clubs-empty">{__kbUi("Mashg‘ulot vaqtini kiriting yoki o‘qituvchining kursini toping.")}</p> : <div className="student-schedule__club-list">{clubs.map(item => <div key={item.id}><b aria-hidden="true">{CLUBS.find(club => club[0] === item.club_type)?.[1] || __kbUi("✦")}</b><span><strong>{item.name}</strong><small>{item.day} · {item.time}{item.note ? __kbUi(` · ${item.note}`) : __kbUi("")}</small></span>{editable && <><button type="button" aria-label={__kbUi(`${item.name}: tahrirlash`)} disabled={loading || saving} onClick={() => openClub(item)}>✎</button><button type="button" aria-label={__kbUi(`${item.name}: o‘chirish`)} disabled={loading || saving} onClick={() => { setNotice(""); setError(""); setDraft(old => { const next = old || clone(data); return { ...next, extracurriculars: next.extracurriculars.filter(club => club.id !== item.id) }; }); }}>×</button></>}</div>)}</div>}</div>
    </>}
