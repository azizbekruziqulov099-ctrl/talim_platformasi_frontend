...
export default function TalabaHaftalikJadval({ token, apiBase, talabaProfili, readOnly = false }) {
  ...
  return (
    <section className={`tj ${tahrir ? "tahrir" : ""}`} aria-label="Haftalik paralar jadvali">
      <header className="tj-bosh">
        <div>
          <span className="tj-kichik">{talabaProfili?.universitet_nomi || "Oliy ta'lim"}</span>
          <h2>Haftalik paralar</h2>
          <p>{sarlavhaQatori}{data ? ` · haftada ${haftalikParalar} para` : ""}</p>
        </div>
        {!readOnly && data && (
          <div className="tj-amallar">
            {tahrir ? (
              <>
                <button type="button" className="tj-ikkilamchi" onClick={() => { setDraft(null); setSozlamaOchiq(false); setXato(""); }} disabled={saqlanmoqda}>Bekor qilish</button>
                <button type="button" className="tj-asosiy" onClick={saqla} disabled={saqlanmoqda}>{saqlanmoqda ? <Loader2 size={15} className="animate-spin" /> : "Saqlash"}</button>
              </>
            ) : (
              <button type="button" className="tj-asosiy" onClick={() => setDraft(clone(data))}>Jadvalni sozlash</button>
            )}
          </div>
        )}
      </header>

      {yuklanmoqda && <p className="tj-holat" role="status"><Loader2 size={15} className="animate-spin" /> Jadval yuklanmoqda…</p>}
      {xato && <p className="tj-xato" role="alert">{xato}</p>}
      {xabar && <p className="tj-holat" role="status">{xabar}</p>}

      {joriy && (
        <>
          <nav className="tj-kunlar" aria-label="Hafta kunlari">
            {korinadiganKunlar.map((k) => (
              <button type="button" key={k.kun} onClick={() => setTanlanganKun(k.kun)}
                ...
                <span className="tj-kun-nomi">{QISQA_KUN[k.kun - 1]}</span>
                <span className="tj-kun-sana">{sanaBelgisi(dushanbaISO, k.kun - 1)}</span>
                <span className="tj-kun-soni">{k.turi === "dam" ? "dam" : k.turi === "amaliyot" ? "amaliyot" : `${k.paralar.length} para`}</span>
          ...
          {kun && (
            <article className={`tj-kun-paneli ${kun.turi}`}>
              <div className="tj-kun-sarlavha">
                <h3>{KUN_NOMLARI[kun.kun - 1]}{kun.kun === bugunKun ? " — bugun" : ""}</h3>
                {tahrir ? (
                  <div className="tj-tur-tanlash" role="group" aria-label="Kun turi">
                    {Object.entries(KUN_TURLARI).map(([k, nom]) => (
                      <button type="button" key={k} className={kun.turi === k ? "on" : ""} onClick={() => kunniYangila(kun.kun, { turi: k })}>{nom}</button>
                    ))}
                  </div>
                ) : <span className={`tj-tur ${kun.turi}`}>{KUN_TURLARI[kun.turi]}</span>}
              </div>

              {kun.turi === "dam" && !tahrir ? (
                <p className="tj-bosh-kun">Dam olish kuni. Paralar yo'q.</p>
              ) : kun.paralar.length === 0 && !tahrir ? (
                <p className="tj-bosh-kun">{kun.turi === "amaliyot" ? "Amaliyot kuni — para kiritilmagan." : "Bu kunga para kiritilmagan."}{!readOnly && " «Jadvalni sozlash» orqali qo'shing."}</p>
              ) : (
                    ...
                    <li key={`${kun.kun}-${i}`} className="tj-para">
                      {tahrir ? (
                        <div className="tj-para-tahrir">
                          <label>Para<select value={p.raqam} onChange={(e) => paraYangila(i, { raqam: Number(e.target.value) })}>{Array.from({ length: 10 }, (_, n) => n).filter((n) => n > 0 || sozlamalar.nol_para || p.raqam === 0).map((n) => <option key={n} value={n}>{n}</option>)}</select></label>
                          <label className="tj-keng">Fan<input value={p.fan} maxLength={120} onChange={(e) => paraYangila(i, { fan: e.target.value })} placeholder="masalan: Boshlang'ich matematika kursi nazariyasi" /></label>
                          <label>Turi<select value={p.turi} onChange={(e) => paraYangila(i, { turi: e.target.value })}>{Object.entries(PARA_TURLARI).map(([k, nom]) => <option key={k} value={k}>{nom}</option>)}</select></label>
                          <label>O'qituvchi<input value={p.oqituvchi} maxLength={120} onChange={(e) => paraYangila(i, { oqituvchi: e.target.value })} placeholder="ixtiyoriy" /></label>
                          <label>Xona<input value={p.xona} maxLength={40} onChange={(e) => paraYangila(i, { xona: e.target.value })} placeholder="masalan: 214" /></label>
                          <label>Vaqt<span className="tj-vaqt-juft"><input type="time" value={p.boshlanish} onChange={(e) => paraYangila(i, { boshlanish: e.target.value })} /><input type="time" value={p.tugash} onChange={(e) => paraYangila(i, { tugash: e.target.value })} /></span><small>Bo'sh qolsa sozlamadan hisoblanadi</small></label>
                          <button type="button" className="tj-ochir" onClick={() => paraOchir(i)} aria-label="Parani o'chirish"><Trash2 size={15} /></button>
                        </div>
                      ) : (
                        <>
                          <span className="tj-para-raqam" aria-label={`${p.raqam}-para`}>{p.raqam}</span>
                          <div className="tj-para-matn">
                            <b>{p.fan}</b>
                            <small>{PARA_TURLARI[p.turi] || p.turi}{p.oqituvchi ? ` · ${p.oqituvchi}` : ""}{p.xona ? ` · ${p.xona}-xona` : ""}{p.izoh ? ` · ${p.izoh}` : ""}</small>
                          </div>
                          <time className="tj-para-vaqt">{paraVaqti(p, sozlamalar)}</time>
                        </>
                      )}
                    </li>
                  ))}
              ...
              {tahrir && kun.turi !== "dam" && (
                <button type="button" className="tj-qosh" onClick={paraQosh} disabled={kun.paralar.length >= 10}><Plus size={15} /> Para qo'shish</button>
              )}
            </article>
          )}
          ...
          {tahrir && (
            <div className="tj-sozlama">
              <button type="button" className="tj-sozlama-tugma" onClick={() => setSozlamaOchiq((v) => !v)} aria-expanded={sozlamaOchiq}><Settings2 size={15} /> Vaqt va hafta sozlamalari</button>
              {sozlamaOchiq && (
                <div className="tj-sozlama-maydonlar">
                  <label>1-para boshlanishi<input type="time" value={sozlamalar.boshlanish || "08:30"} onChange={(e) => setDraft((old) => ({ ...old, sozlamalar: { ...old.sozlamalar, boshlanish: e.target.value } }))} /></label>
                  <label>Para davomiyligi (daqiqa)<input type="number" min={30} max={180} value={sozlamalar.para_daqiqa ?? 80} onChange={(e) => setDraft((old) => ({ ...old, sozlamalar: { ...old.sozlamalar, para_daqiqa: Number(e.target.value) } }))} /></label>
                  <label>Tanaffus (daqiqa)<input type="number" min={0} max={60} value={sozlamalar.tanaffus_daqiqa ?? 10} onChange={(e) => setDraft((old) => ({ ...old, sozlamalar: { ...old.sozlamalar, tanaffus_daqiqa: Number(e.target.value) } }))} /></label>
                  <label>Haftada kunlar<select value={sozlamalar.kunlar_soni || 6} onChange={(e) => setDraft((old) => ({ ...old, sozlamalar: { ...old.sozlamalar, kunlar_soni: Number(e.target.value) } }))}><option value={5}>5 kun (Du–Ju)</option><option value={6}>6 kun (Du–Sha)</option><option value={7}>7 kun</option></select></label>
                  <label className="tj-belgi"><input type="checkbox" checked={!!sozlamalar.nol_para} onChange={(e) => setDraft((old) => ({ ...old, sozlamalar: { ...old.sozlamalar, nol_para: e.target.checked } }))} /> 0-para ham bor</label>
                </div>
              )}
            </div>
          )}
