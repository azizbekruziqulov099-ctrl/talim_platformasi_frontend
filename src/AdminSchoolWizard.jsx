  ...
  return <div className="relative">
    ...
    {results.length > 0 && <div className="absolute z-30 left-0 right-0 mt-1 rounded-xl border bg-white shadow-lg p-1 max-h-44 overflow-auto" style={{ borderColor: "#E5E1D8" }}>
      {results.map((person) => <button type="button" key={person.user_id} onClick={() => { onChange(person); setQuery(""); setResults([]); }} className="w-full text-left rounded-lg px-3 py-2 text-xs hover:bg-slate-50">
        <b>{person.full_name}</b><span className="block" style={{ color: "#8A8578" }}>{__kbUi(person.role)}{__kbUi(" · ID ")}{person.user_id}</span>
      </button>)}
    </div>}
  </div>;
}
...
export default function AdminSchoolWizard({ token, apiBase, regions, districtsByRegion, onCancel, onCreated }) {
  ...
  const [schoolKind, setSchoolKind] = useState("oddiy");
    ...
    try {
      const response = await fetch(`${apiBase}/api/admin/maktab-yaratish-v2`, {
        ...
        body: JSON.stringify({
          ...
          maktab_turi: schoolKind, shift_count: shiftCount, director_user_id: director?.user_id || null,
  ...
  return <section className="rounded-2xl p-5 bg-white border mb-4" style={{ borderColor: "#D9D4C8" }}>
    <div className="flex items-start justify-between gap-3 mb-4"><div>
      <p className="text-xs font-bold" style={{ color: "#8A5A1C" }}>{__kbUi("YANGI MAKTAB · ")}{step}{__kbUi("/4 BOSQICH")}</p>
      <h2 className="text-lg font-bold" style={{ color: "#21384C" }}>{step === 1 ? __kbUi("Maktab ma’lumoti") : step === 2 ? __kbUi("Bino va xonalar") : step === 3 ? __kbUi("Sinflarni tez yaratish") : __kbUi("Tekshirish va yaratish")}</h2>
    </div><button type="button" onClick={onCancel} className="px-3 py-1.5 rounded-lg text-xs font-semibold" style={{ background: "#F7F5F0", color: "#5A5648" }}>{__kbUi("✕ Yopish")}</button></div>
    ...
    {step === 1 && <div className="space-y-3">
      <label className="block text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Maktab turi")}<select value={schoolKind} onChange={(event) => setSchoolKind(event.target.value)} className="block w-full mt-1.5 px-3.5 py-2.5 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}><option value="oddiy">{__kbUi("Umumta’lim maktabi")}</option><option value="harbiy">{__kbUi("Harbiy maktab")}</option></select>
      </label>
      {schoolKind === "harbiy" && <p className="text-xs rounded-xl p-3" style={{ background: "#EEF7F4", color: "#22594F" }}>{__kbUi("Kun tartibini maktab ochilgach rahbariyat bosh sahifasida kiriting. Ota-onalar va o‘quvchilar tasdiqlangan vaqtlarni shu yerda ko‘radi.")}</p>}
      <div className="grid md:grid-cols-2 gap-3">
        <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Maktab nomi · ixtiyoriy")}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={__kbUi("Masalan: Alisher Navoiy nomidagi / Prezident ixtisoslashgan")} className="block w-full mt-1.5 px-3.5 py-2.5 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }} /></label>
        <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Maktab raqami *")}<input value={schoolNumber} inputMode="numeric" onChange={(event) => setSchoolNumber(event.target.value.replace(/\D/g, "").slice(0, 5))} placeholder={__kbUi("Masalan: 21")} className="block w-full mt-1.5 px-3.5 py-2.5 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }} /></label>
      </div>
      <div className="grid md:grid-cols-2 gap-3">
        <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Viloyat *")}<select value={region} onChange={(event) => { setRegion(event.target.value); setDistrict(""); }} className="block w-full mt-1.5 px-3.5 py-2.5 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}><option value="">{__kbUi("Tanlang")}</option>{(regions || []).map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Tuman/shahar *")}<select value={district} onChange={(event) => setDistrict(event.target.value)} disabled={!region} className="block w-full mt-1.5 px-3.5 py-2.5 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8", opacity: region ? 1 : 0.55 }}><option value="">{__kbUi("Tanlang")}</option>{((districtsByRegion || {})[region] || []).map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      </div>
      <div><p className="text-xs font-semibold mb-1.5" style={{ color: "#5A5648" }}>{__kbUi("Maktabdagi smena soni *")}</p><div className="grid grid-cols-2 gap-2">{[1, 2].map((number) => <button type="button" key={number} onClick={() => {
        ...
        setClassesPlanDirty(true);
      }} className="py-2.5 rounded-xl border text-sm font-bold" style={shiftCount === number ? { background: "#1B4B7A", color: "white", borderColor: "#1B4B7A" } : { background: "white", color: "#5A5648", borderColor: "#E5E1D8" }}>{__kbUi(number)}{__kbUi(" smenali")}</button>)}</div></div>
      <label className="text-xs font-semibold block" style={{ color: "#5A5648" }}>{__kbUi("Direktor · ixtiyoriy")}<div className="mt-1.5"><PersonPicker token={token} apiBase={apiBase} value={director} onChange={setDirector} placeholder={__kbUi("Mavjud foydalanuvchidan direktor tanlang...")} /></div></label>
      <div className="rounded-xl px-3.5 py-3 text-xs" style={{ background: "#EEF6F1", color: "#2E6C55" }}>{__kbUi("Admin yaratmoqda: platforma to‘lovi, balans va sinov muddati so‘ralmaydi.")}</div>
    </div>}
    ...
    {step === 2 && <div className="space-y-3">
      ...
      }} className="mt-0.5" /><span className="text-xs" style={{ color: "#5A5648" }}><b>{__kbUi("Bino va xonalarni keyin kiritaman")}</b><small className="block mt-0.5">{__kbUi("Belgilanmasa, hozir bino va xonalar to‘liq yaratiladi.")}</small></span></label>
      {!skipBuildings && buildings.map((building, index) => <article key={building.key} className="rounded-2xl border p-4" style={{ borderColor: "#E5E1D8", background: "#FCFBF8" }}>
        <div className="flex items-center justify-between mb-3"><b className="text-sm" style={{ color: "#21384C" }}>{building.name.trim() || __kbUi(`${index + 1}-bino`)}</b>{buildings.length > 1 && <button type="button" onClick={() => removeBuilding(building.key)} className="text-xs" style={{ color: "#B0553A" }}>{__kbUi("Olib tashlash")}</button>}</div>
        <div className="grid md:grid-cols-4 gap-3">
          <label className="text-xs font-semibold md:col-span-2" style={{ color: "#5A5648" }}>{__kbUi("Bino nomi *")}<input value={building.name} onChange={(event) => updateBuilding(building.key, { name: event.target.value, rooms: [] })} placeholder={__kbUi("Masalan: Asosiy bino")} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }} /></label>
          <label className="text-xs font-semibold md:col-span-2" style={{ color: "#5A5648" }}>{__kbUi("Qavat soni *")}<input type="number" min="1" max="20" value={building.floors} onChange={(event) => updateBuildingFloors(building.key, event.target.value)} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }} /></label>
          <div className="md:col-span-4"><p className="text-xs font-semibold mb-2" style={{ color: "#5A5648" }}>{__kbUi("Har bir qavatdagi xona soni *")}</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2">{__kbUi(Array.from({ length: Number(building.floors) || 1 }, (_, floorIndex) => <label key={floorIndex} className="text-[11px] font-semibold rounded-xl border p-2.5" style={{ color: "#5A5648", borderColor: "#E5E1D8", background: "white" }}>{floorIndex + 1}{__kbUi("-qavat")}<input aria-label={__kbUi(`${floorIndex + 1}-qavat xona soni`)} type="number" min="0" max="100" value={building.floorRoomCounts?.[floorIndex] ?? building.roomsPerFloor ?? 10} onChange={(event) => updateFloorRoomCount(building.key, floorIndex, event.target.value)} className="block w-full mt-1.5 px-3 py-2 rounded-lg border text-sm" style={{ borderColor: "#D9D4C8" }} /></label>))}</div></div>
          <label className="text-xs font-semibold md:col-span-2" style={{ color: "#5A5648" }}>{__kbUi("Xona raqamlash usuli")}<select value={building.scheme} onChange={(event) => updateBuilding(building.key, { scheme: event.target.value, rooms: [] })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}><option value="floor">{__kbUi("Qavat bo‘yicha: 101, 102… 201, 202…")}</option><option value="sequential">{__kbUi("Oddiy ketma-ket: 1, 2, 3…")}</option></select></label>
          <label className="text-xs font-semibold md:col-span-2" style={{ color: "#5A5648" }}>{__kbUi("Dars o‘tilmaydigan xonalar · ixtiyoriy")}<input value={building.customRooms} onChange={(event) => updateBuilding(building.key, { customRooms: event.target.value, rooms: [] })} placeholder={__kbUi("Masalan: Sportzal, Kutubxona, Oshxona")} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }} /><small className="block mt-1 font-normal">{__kbUi("Vergul, nuqtali vergul yoki yangi qator bilan ajrating. Bu xonalar sinflarga biriktirilmaydi.")}</small></label>
        </div>
        <button type="button" onClick={() => createRooms(building.key)} className="w-full mt-3 py-2.5 rounded-xl text-sm font-bold text-white" style={{ background: "#1B4B7A" }}>⚡ {building.rooms.length ? __kbUi("Xonalarni qayta yaratish") : __kbUi("Xonalarni avtomatik yaratish")}</button>
        {building.rooms.length > 0 && <div className="mt-3 rounded-xl p-3" style={{ background: "#F1F7FB" }}><p className="text-xs font-bold mb-1" style={{ color: "#1B4B7A" }}>{building.rooms.filter(isTeachingRoom).length}{__kbUi(" ta o‘quv xonasi · ")}{building.rooms.filter((room) => !isTeachingRoom(room)).length}{__kbUi(" ta dars o‘tilmaydigan xona")}</p><p className="text-[11px] mb-2" style={{ color: "#5A5648" }}>{__kbUi(Array.from({ length: Number(building.floors) || 1 }, (_, floorIndex) => `${floorIndex + 1}-qavat: ${building.rooms.filter((room) => room.floor === floorIndex + 1).length} xona`).join(" · "))}</p><div className="flex flex-wrap gap-1.5 max-h-24 overflow-auto">{building.rooms.map((room) => <span key={`${building.key}-${room.number}`} className="px-2 py-1 rounded-lg text-[11px]" style={{ background: isTeachingRoom(room) ? "white" : "#FFF0EC", color: isTeachingRoom(room) ? "#5A5648" : "#A04431" }}>{room.number}{isTeachingRoom(room) ? __kbUi("") : __kbUi(" · dars yo‘q")}</span>)}</div></div>}
      </article>)}
      {!skipBuildings && <button type="button" onClick={() => setBuildings((current) => [...current, emptyBuilding(current.length)])} className="w-full py-3 rounded-xl border-2 border-dashed text-sm font-bold" style={{ borderColor: "#B9CCDC", color: "#1B4B7A" }}>{__kbUi("＋ Yana bino qo‘shish")}</button>}
    </div>}

    {step === 3 && <div className="space-y-4">
      <section className="rounded-2xl border p-4" style={{ borderColor: "#D9D4C8", background: "#FCFBF8" }}>
        <div className="flex items-start justify-between gap-3 mb-3"><div><b className="text-sm" style={{ color: "#21384C" }}>{__kbUi("⚡ Sinflarni tez yaratish")}</b><p className="text-xs mt-1" style={{ color: "#8A8578" }}>{__kbUi("Tilni tanlang, har darajaga nechta sinf bo‘lishini yozing. Har til o‘z alifbosida nomlanadi.")}</p></div><span className="text-xs font-bold px-2.5 py-1 rounded-full whitespace-nowrap" style={{ background: "#EAF1F7", color: "#1B4B7A" }}>{requestedClassCount}{__kbUi(" ta reja")}</span></div>
        <div className="grid grid-cols-3 gap-2 mb-3">{CLASS_LANGUAGES.map((lang) => {
          ...
          return <button type="button" key={lang.code} onClick={() => setLanguageTab(lang.code)} className="py-2.5 rounded-xl border text-sm font-bold" style={active ? { background: "#1B4B7A", color: "white", borderColor: "#1B4B7A" } : { background: "white", color: "#5A5648", borderColor: "#E5E1D8" }}>{lang.tab}<span className="block text-[10px] font-semibold mt-0.5" style={{ opacity: 0.85 }}>{total ? __kbUi(`${total} ta sinf`) : __kbUi("yo‘q")} · {__kbUi(lang.letters.slice(0, 3).join(", "))}...</span></button>;
        })}</div>
        <div className="flex flex-wrap items-center gap-2 mb-3"><span className="text-[11px] font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Harflar qaysi harfdan boshlansin:")}</span><select value={startLetters[languageTab]} onChange={(event) => { const letter = event.target.value; setStartLetters((current) => ({ ...current, [languageTab]: letter })); setClassesPlanDirty(true); }} className="px-2 py-1.5 rounded-lg border text-sm font-bold" style={{ borderColor: "#D9D4C8", color: "#21384C" }} title={__kbUi("Boshlang‘ich harf")}>{lettersOf(languageTab).map((letter) => <option key={letter} value={letter}>{letter}{languageOf(languageTab).suffix}</option>)}</select><span className="text-[10px]" style={{ color: "#8A8578" }}>{__kbUi("Masalan rus sinflari В dan boshlansin desangiz — В ni tanlang.")}</span></div>
        <div className="flex flex-wrap items-center gap-1.5 mb-3"><span className="text-[11px] font-semibold mr-1" style={{ color: "#5A5648" }}>{__kbUi("Barcha darajaga ")}{__kbUi(languageLabel(languageTab).toLocaleLowerCase("uz"))}{__kbUi(" sinfi:")}</span>{[1, 2, 3, 5, 8].map((count) => <button type="button" key={count} onClick={() => applyParallelPreset(count)} className="px-2.5 py-1.5 rounded-lg text-[11px] font-bold" style={{ background: "#EAF1F7", color: "#1B4B7A" }}>{__kbUi(count)}{__kbUi(" tadan")}</button>)}<button type="button" onClick={() => applyParallelPreset(0)} className="px-2.5 py-1.5 rounded-lg text-[11px] font-semibold" style={{ background: "#FFF0EC", color: "#B0553A" }}>{__kbUi("Tozalash")}</button></div>
        <div className="rounded-xl border overflow-hidden mb-3" style={{ borderColor: "#E5E1D8" }}>
          <div className="grid grid-cols-12 gap-2 px-3 py-2 text-[10px] font-bold" style={{ background: "#F1F7FB", color: "#5A5648" }}><span className="col-span-2">{__kbUi("DARAJA")}</span><span className="col-span-2 text-center">{__kbUi("1-SMENA")}</span>{shiftCount === 2 && <span className="col-span-2 text-center">{__kbUi("2-SMENA")}</span>}<span className={shiftCount === 2 ? "col-span-6" : "col-span-8"}>{__kbUi("YARATILADIGAN SINFLAR")}</span></div>
          {gradeConfig.map((item) => {
            ...
            return <div key={item.grade} className="grid grid-cols-12 gap-2 items-center px-3 py-2 border-t" style={{ borderColor: "#F0ECE3", background: plan.length ? "white" : "#FAF9F6" }}>
              <b className="col-span-2 text-xs" style={{ color: "#21384C" }}>{item.grade}{__kbUi("-sinf")}</b>
              {__kbUi(countInput(1, "#1B4B7A"))}
              {shiftCount === 2 && __kbUi(countInput(2, "#8A5A1C"))}
              <span className={`${shiftCount === 2 ? "col-span-6" : "col-span-8"} text-[11px] truncate`} title={preview} style={{ color: plan.length ? "#5A5648" : "#A8A397" }}>{plan.length ? preview : __kbUi("Yaratilmaydi")}</span>
            </div>;
          })}
        </div>
        <p className="text-[11px] mb-3" style={{ color: "#8A8578" }}>{__kbUi("O‘zbek sinflari: 1-A, 1-B, 1-D... · Rus sinflari: 1-А, 1-Б, 1-В (kirill) · Ingliz sinflari: 1-A-EN, 1-B-EN... Pastdagi ro‘yxatda istalgan sinfning harfini o‘zgartirish mumkin.")}</p>
        <button type="button" onClick={generateClasses} className="w-full py-3 rounded-xl text-sm font-bold text-white" style={{ background: "#1B4B7A" }}>⚡ {requestedClassCount}{__kbUi(" ta sinfni qayta hisoblash va yaratish")}</button>
      </section>
      {classes.length > 0 && <>
        {!skipBuildings && roomPool.length > 0 && <button type="button" onClick={autoAssignAllRooms} className="w-full py-2.5 rounded-xl text-sm font-bold" style={{ background: "#EEF6F1", color: "#2E6C55" }}>{__kbUi("🏫 Xonalarni smena bo‘yicha biriktirish")}</button>}
        <div className="flex items-center justify-between gap-2"><div><b className="text-sm" style={{ color: "#21384C" }}>{__kbUi("Yaratiladigan sinflar")}</b><p className="text-[11px] mt-0.5" style={{ color: "#8A8578" }}>{__kbUi("Kerakli sinfni bosing: uning smena, bino, xona, rahbar va psixologi alohida ochiladi.")}</p></div><button type="button" onClick={() => { setClasses([]); setNotice(""); }} className="text-xs whitespace-nowrap" style={{ color: "#B0553A" }}>{__kbUi("Ro‘yxatni tozalash")}</button></div>
        <div className="space-y-2">{sortedClasses(classes).map((item) => {
          const selectedBuilding = buildingByKey.get(item.buildingKey);
          return <details key={item.key} className="rounded-xl border bg-white overflow-visible" style={{ borderColor: "#E5E1D8" }}><summary className="px-3.5 py-3 flex items-center gap-3 cursor-pointer [&::-webkit-details-marker]:hidden" style={{ listStyle: "none" }}><b className="w-8 text-sm" style={{ color: "#21384C" }}>{item.grade}-</b><select aria-label={__kbUi("Sinf harfi")} value={String(item.letter || "").toUpperCase()} onClick={(event) => event.preventDefault()} onChange={(event) => updateClass(item.key, { letter: event.target.value })} className="w-16 px-1 py-1 rounded-lg border text-sm font-bold text-center" style={{ borderColor: "#D9D4C8", color: "#21384C" }} title={__kbUi("Harfni o‘zgartirish")}>{lettersOf(item.language).map((letter) => <option key={letter} value={letter}>{letter}{languageOf(item.language).suffix}</option>)}</select><select aria-label={__kbUi("Ta‘lim tili")} value={item.language || "uz"} onClick={(event) => event.preventDefault()} onChange={(event) => { const nextLanguage = event.target.value; const index = Math.max(0, letterIndex(item.letter, item.language)); updateClass(item.key, { language: nextLanguage, letter: lettersOf(nextLanguage)[Math.min(index, lettersOf(nextLanguage).length - 1)] }); }} className="px-1.5 py-1 rounded-lg border text-[11px] font-bold" style={{ borderColor: "#D9D4C8", color: "#1B4B7A", background: "#EAF1F7" }} title={__kbUi("Ta‘lim tilini o‘zgartirish")}>{CLASS_LANGUAGES.map((lang) => <option key={lang.code} value={lang.code}>{__kbUi(lang.label)}</option>)}</select><span className="flex-1 text-xs truncate" style={{ color: "#8A8578" }}>{item.shift}{__kbUi("-smena · ")}{selectedBuilding ? __kbUi(`${selectedBuilding.name}, ${item.roomNumber || "xona tanlanmagan"}`) : __kbUi("bino/xona tanlanmagan")}</span><span style={{ color: "#8A8578" }}>⌄</span></summary>
            <div className="border-t p-3.5 grid md:grid-cols-3 gap-3" style={{ borderColor: "#F0ECE3" }}>
              <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Sinf darajasi *")}<select value={item.grade} onChange={(event) => updateClass(item.key, { grade: event.target.value })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}>{CLASS_GRADES.map((grade) => <option key={grade} value={grade}>{grade}{__kbUi("-sinf")}</option>)}</select></label>
              <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Sinf parallel harfi *")}<select value={String(item.letter || "").toUpperCase()} onChange={(event) => updateClass(item.key, { letter: event.target.value })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}>{lettersOf(item.language).map((letter) => <option key={letter} value={letter}>{letter}{languageOf(item.language).suffix}</option>)}</select></label>
              <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Ta‘lim tili *")}<select value={item.language || "uz"} onChange={(event) => updateClass(item.key, { language: event.target.value })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}>{CLASS_LANGUAGES.map((lang) => <option key={lang.code} value={lang.code}>{__kbUi(lang.label)}</option>)}</select></label>
              {shiftCount === 2 && <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Smena *")}<select value={item.shift} onChange={(event) => updateClass(item.key, { shift: Number(event.target.value), buildingKey: "", roomNumber: "" })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}><option value={1}>{__kbUi("1-smena")}</option><option value={2}>{__kbUi("2-smena")}</option></select></label>}
              {!skipBuildings && <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Bino · xona bilan birga ixtiyoriy")}<select value={item.buildingKey} onChange={(event) => updateClass(item.key, { buildingKey: event.target.value, roomNumber: "" })} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8" }}><option value="">{__kbUi("Tanlanmagan")}</option>{buildings.map((building) => <option key={building.key} value={building.key}>{building.name}</option>)}</select></label>}
              {!skipBuildings && <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Xona · bino bilan birga ixtiyoriy")}<select value={item.roomNumber} onChange={(event) => assignClassRoom(item, event.target.value)} disabled={!selectedBuilding} className="block w-full mt-1.5 px-3 py-2 rounded-xl border text-sm" style={{ borderColor: "#E5E1D8", opacity: selectedBuilding ? 1 : 0.55 }}><option value="">{__kbUi("Tanlanmagan")}</option>{(selectedBuilding?.rooms || []).map((room) => {
                ...
                return <option key={room.number} value={room.number} disabled={unavailable}>{room.number}{__kbUi("-xona")}{!isTeachingRoom(room) ? __kbUi(" · dars o‘tilmaydi") : usedByAnotherClass ? __kbUi(` · ${classNameOf(owner)} band`) : __kbUi("")}</option>;
              })}</select></label>}
              <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Sinf rahbari · ixtiyoriy")}<div className="mt-1.5"><PersonPicker token={token} apiBase={apiBase} value={item.leader} onChange={(person) => updateClass(item.key, { leader: person })} placeholder={__kbUi("Rahbar ismi...")} /></div></label>
              <label className="text-xs font-semibold" style={{ color: "#5A5648" }}>{__kbUi("Psixolog · ixtiyoriy")}<div className="mt-1.5"><PersonPicker token={token} apiBase={apiBase} value={item.psychologist} onChange={(person) => updateClass(item.key, { psychologist: person })} placeholder={__kbUi("Psixolog ismi...")} /></div></label>
              <button type="button" onClick={() => setClasses((current) => current.filter((row) => row.key !== item.key))} className="self-end py-2 rounded-xl text-xs font-semibold" style={{ background: "#FFF0EC", color: "#B0553A" }}>{__kbUi("Sinfni olib tashlash")}</button>
            </div></details>;
        })}</div>
      </>}
    </div>}
    ...
    {step === 4 && <div className="space-y-4">
      <div className="rounded-2xl p-4" style={{ background: "#F7F5F0" }}><h3 className="font-bold" style={{ color: "#21384C" }}>{name.trim() ? __kbUi(`${schoolNumber.trim()}-sonli ${name.trim()}`) : __kbUi(`${schoolNumber.trim()}-maktab`)}</h3><p className="text-xs mt-1" style={{ color: "#5A5648" }}>{region}, {district} · {schoolKind === "harbiy" ? __kbUi("Harbiy maktab") : __kbUi("Umumta’lim maktabi")} · {shiftCount}{__kbUi(" smenali · ")}{director ? __kbUi(`Direktor: ${director.full_name}`) : __kbUi("Direktor keyin belgilanadi")}</p></div>
      <div className="grid grid-cols-3 gap-2"><div className="rounded-xl p-3 text-center" style={{ background: "#F1F7FB" }}><b className="block text-lg" style={{ color: "#1B4B7A" }}>{skipBuildings ? 0 : buildings.length}</b><span className="text-xs" style={{ color: "#5A5648" }}>{__kbUi("bino")}</span></div><div className="rounded-xl p-3 text-center" style={{ background: "#F1F7FB" }}><b className="block text-lg" style={{ color: "#1B4B7A" }}>{roomPool.length}</b><span className="text-xs" style={{ color: "#5A5648" }}>{__kbUi("xona")}</span></div><div className="rounded-xl p-3 text-center" style={{ background: "#FDF3E0" }}><b className="block text-lg" style={{ color: "#8A5A1C" }}>{classes.length}</b><span className="text-xs" style={{ color: "#5A5648" }}>{__kbUi("sinf")}</span></div></div>
      {!skipBuildings && buildings.map((building) => <div key={building.key} className="rounded-xl border px-3.5 py-3" style={{ borderColor: "#E5E1D8" }}><b className="text-sm">{building.name}</b><p className="text-xs mt-1" style={{ color: "#8A8578" }}>{building.floors}{__kbUi(" qavat · ")}{building.rooms.length}{__kbUi(" xona")}</p><p className="text-[11px] mt-1" style={{ color: "#5A5648" }}>{__kbUi(Array.from({ length: Number(building.floors) || 1 }, (_, floorIndex) => `${floorIndex + 1}-qavat: ${building.rooms.filter((room) => room.floor === floorIndex + 1).length} xona`).join(" · "))}</p></div>)}
      <div className="rounded-xl border max-h-72 overflow-auto" style={{ borderColor: "#E5E1D8" }}>{sortedClasses(classes).map((item) => { const building = buildingByKey.get(item.buildingKey); return <div key={item.key} className="px-3.5 py-2.5 border-b last:border-b-0 flex items-center gap-3" style={{ borderColor: "#F0ECE3" }}><b className="w-12 text-sm">{__kbUi(classNameOf(item))}</b><span className="text-xs flex-1" style={{ color: "#8A8578" }}>{__kbUi(languageLabel(item.language))} · {item.shift}{__kbUi("-smena · ")}{building ? __kbUi(`${building.name}, ${item.roomNumber || "xonasiz"}`) : __kbUi("bino/xonasiz")}</span><span className="text-[11px]" style={{ color: "#5A5648" }}>{item.leader?.full_name || __kbUi("rahbarsiz")}</span></div>; })}</div>
      <div className="rounded-xl px-3.5 py-3 text-xs font-semibold" style={{ background: "#EEF6F1", color: "#2E6C55" }}>{__kbUi("Maktab, binolar, xonalar va sinflar bitta xavfsiz amalda yaratiladi. Platforma to‘lovi: 0 so‘m.")}</div>
    </div>}

    {notice && <div className="mt-4 rounded-xl px-3.5 py-3 text-xs" role="status" style={{ background: "#EEF6F1", color: "#2E6C55" }}>✓ {__kbUi(notice)}</div>}
    {error && <div className="mt-4 rounded-xl px-3.5 py-3 text-sm" role="alert" style={{ background: "#FFF0EC", color: "#A04431" }}>{__kbUi(error)}</div>}
    <div className="grid grid-cols-2 gap-2 mt-5"><button type="button" onClick={() => step === 1 ? onCancel?.() : setStep((current) => current - 1)} disabled={saving} className="py-3 rounded-xl font-bold text-sm" style={{ background: "#F7F5F0", color: "#5A5648" }}>{step === 1 ? __kbUi("Bekor qilish") : __kbUi("← Orqaga")}</button>{step < 4 ? <button type="button" onClick={goNext} className="py-3 rounded-xl font-bold text-sm text-white" style={{ background: "#1B4B7A" }}>{__kbUi("Davom etish →")}</button> : <button type="button" onClick={createSchool} disabled={saving} className="py-3 rounded-xl font-bold text-sm text-white" style={{ background: "#1B4B7A", opacity: saving ? 0.65 : 1 }}>{saving ? __kbUi("Yaratilmoqda...") : __kbUi("Hammasini yaratish")}</button>}</div>
  </section>;
}
