...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, onOpenStaff, startMode, onStartModeConsumed, initialFacultyId = null }) {
  ...
  useEffect(() => {
    ...
    const faculty = (data.fakultetlar || []).find(x => String(x.id) === String(initialFacultyId));
    if (faculty) setSelectedFaculty(faculty);
  }, [data, initialFacultyId, selectedFaculty]);
  useEffect(() => {
    if (!startMode) return;
    setMode(startMode);
    onStartModeConsumed?.();
...
function StaffPanel({ api, token, universityId, structure, canManage, canManageAdmins, isSuperAdmin, onCredentials }) {
  ...
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const load = useCallback(async () => { try { const d = await api(`/api/institut/v20/xodimlar?universitet_id=${universityId}&arxiv=${archiveMode ? "true" : "false"}&token=${encodeURIComponent(token)}`); setStaff(d.xodimlar || []); } catch (e) { setError(e.message); } }, [api, token, universityId, archiveMode]);
  useEffect(() => { load(); }, [load]);
...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null, lockedProgramIds = null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (lockedProgramIds?.length) qs.set("yonalish_ids", lockedProgramIds.join(","));
      else if (lockedProgramId) qs.set("yonalish_id", String(lockedProgramId));
  ...
  }, [api, universityId, token, tab, page, filters, selectedFacultyId, lockedProgramId, lockedProgramIds, permissions.qabul_korish]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2"><Button onClick={() => setMode("list")} kind={mode === "list" ? "primary" : "secondary"}><Users size={16} /> {lockedProgramId ? "Yo‘nalish talabalari" : "Talabalar"}</Button>{permissions.hujjat_belgilash && lockedFacultyId && !lockedProgramId && <Button onClick={() => setMode("import")} kind={mode === "import" ? "primary" : "secondary"}><FileSpreadsheet size={16} /> Talabalarni import qilish</Button>}</div>
...
function TutorPanel({ api, token, universityId, structure, canManage }) {
  ...
  const [form, setForm] = useState({ tyutor_user_id: "", fakultet_id: "", yonalish_id: "", talim_shakli: "", talim_tili: "", qabul_turi: "" });
  ...
  const assign = async () => {
    ...
    try {
      ...
      setForm({ tyutor_user_id: "", fakultet_id: "", yonalish_id: "", talim_shakli: "", talim_tili: "", qabul_turi: "" }); setSelectedProgramIds([]); load();
    } catch (e) { setError(e.message); }
  };
