...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, onOpenStaff, startMode, onStartModeConsumed }) {
  ...
  if (selectedFaculty && selectedProgram) return <div className="space-y-4">
    ...
    <AdmissionsPanel api={api} apiBase={apiBase} token={token} universityId={universityId} structure={data} permissions={permissions} onCredentials={onCredentials} startMode="list" lockedFacultyId={selectedFaculty.id} lockedProgramId={selectedProgram.id} />
  </div>;
...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (lockedProgramId) qs.set("yonalish_id", String(lockedProgramId));
  ...
  }, [api, universityId, token, tab, page, filters, selectedFacultyId, lockedProgramId, permissions.qabul_korish]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
