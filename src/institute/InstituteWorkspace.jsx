...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false }) {
  ...
  const [tab, setTab] = useState("dashboard"); const [structureStartMode, setStructureStartMode] = useState(null); const [admissionStartMode, setAdmissionStartMode] = useState(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const [credentials, setCredentials] = useState([]); const [manualUniversityId, setManualUniversityId] = useState(null);
  ...
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
      ...
      {tab === "dashboard" && <div className="space-y-4">{permissions.tuzilma_boshqarish && <Card className="p-5 md:p-6"><Pill tone="violet">TUZILMANI KIRITISH</Pill><h2 className="mt-3 text-xl font-black" style={{ color: COLORS.ink }}>Qo‘lda yoki tayyor XLSX shablon orqali</h2><p className="mb-5 mt-1 text-sm" style={{ color: COLORS.muted }}>Sizga qulay usulni tanlang. Shablon importida ma’lumotlar avval tekshiriladi.</p><StructureEntryChoices onManual={() => openStructure("manual")} onImport={() => openStructure("import")} /></Card>}<div className="grid gap-4 lg:grid-cols-3"><Card className="p-5 lg:col-span-2"><h2 className="text-xl font-black" style={{ color: COLORS.ink }}>Tizim tayyorligi</h2><div className="mt-4 grid gap-3 md:grid-cols-3">{[["Fakultet va kafedralar", bootstrap.sonlar.fakultet > 0], ["Ta’lim yo‘nalishlari", bootstrap.sonlar.yonalish > 0], ["1-kurs qabul bazasi", bootstrap.sonlar.talaba > 0]].map(([l, ok]) => <div key={l} className="rounded-2xl p-4" style={{ background: ok ? "#ECF7F0" : "#FFF6E8" }}><div className="flex items-center gap-2 font-black" style={{ color: ok ? COLORS.green : COLORS.amber }}>{ok ? <CheckCircle2 size={18} /> : <RefreshCcw size={18} />}{l}</div></div>)}</div><div className="mt-5 rounded-2xl border p-4 text-sm" style={{ borderColor: COLORS.line, color: COLORS.muted }}>Ketma-ketlik: <b>1) Tuzilma</b> → <b>2) Xodim va rollar</b> → <b>3) Qabul importi</b> → <b>4) Tyutorlarni yo‘nalishga biriktirish</b>. Guruhlar HEMISga o‘tkazilgandan keyin alohida bosqichda ochiladi.</div></Card><AccessPanel api={api} token={token} onRedeemed={() => load()} /></div></div>}
