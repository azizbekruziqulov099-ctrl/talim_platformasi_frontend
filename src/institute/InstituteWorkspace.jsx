...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null, lockedProgramIds = null, compactProgram = false }) {
  ...
  return <div className="space-y-4">
    {!compactProgram && <div className="flex flex-wrap gap-2"><Button onClick={() => setMode("list")} kind={mode === "list" ? "primary" : "secondary"}><Users size={16} /> Talabalar</Button>{canViewReports && <Button onClick={() => setMode("reports")} kind={mode === "reports" ? "primary" : "secondary"}><ClipboardCheck size={16} /> Hujjat qabuli hisoboti</Button>}{permissions.hujjat_belgilash && !lockedProgramId && <Button onClick={() => setMode("import")} kind={mode === "import" ? "primary" : "secondary"}><FileSpreadsheet size={16} /> Talabalarni import qilish</Button>}{permissions.hujjat_belgilash && <Button onClick={() => setMode("hujjat_import")} kind={mode === "hujjat_import" ? "primary" : "secondary"}><CheckCircle2 size={16} /> Hujjat topshirganlarni import</Button>}</div>}
    {mode === "hujjat_import" && permissions.hujjat_belgilash && <DocumentImportPanel apiBase={apiBase} token={token} universityId={universityId} onDone={() => { load({ silent: true }); }} />}
...
function DocumentImportPanel({ apiBase, token, universityId, onDone }) {
  ...
  const send = async confirm => {
    ...
    try {
      ...
      const r = await fetch(`${apiBase}/api/institut/v20/qabul/hujjat_import?universitet_id=${universityId}&token=${encodeURIComponent(token)}&tasdiqlash=${confirm ? 1 : 0}`, { method: "POST", body: form });
  ...
  return <Card className="p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><Pill tone="amber">OMMAVIY BELGILASH</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>Hujjat topshirganlarni Excel orqali belgilash</h3><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>Shablonni yuklang → A ustuniga hujjat topshirgan abituriyentlarning <b>AbiturID</b> larini qo‘ying → faylni yuklang. Avval tekshiriladi, keyin siz tasdiqlaysiz.</p></div>
    ...
    {preview && <div className="mt-4 space-y-3">
      <div className="grid gap-3 sm:grid-cols-4">{[["Fayldagi ID", preview.jami_id, "blue"], ["Belgilanadi", preview.belgilanadi, "green"], ["Allaqachon belgilangan", preview.allaqachon, "amber"], ["Topilmadi", preview.topilmadi, "red"]].map(([label, n, tone]) => <div key={label} className="rounded-2xl border p-3" style={{ borderColor: COLORS.line }}><Pill tone={tone}>{label}</Pill><div className="mt-1 text-2xl font-black" style={{ color: COLORS.ink }}>{n}</div></div>)}</div>
      {preview.belgilanadi > 0 && <div className="rounded-2xl border p-3" style={{ borderColor: COLORS.line }}><div className="mb-2 text-xs font-black" style={{ color: COLORS.green }}>Belgilanadi ({preview.belgilanadi}):</div><div className="max-h-48 overflow-auto text-xs" style={{ color: COLORS.ink }}>{preview.belgilanadi_royxat.map((r, i) => <div key={r.abitur_id + i} className="flex justify-between gap-2 border-t py-1" style={{ borderColor: COLORS.line }}><span>{r.abitur_id} · <b>{r.fish}</b></span><span style={{ color: COLORS.muted }}>{r.yonalish}</span></div>)}{preview.belgilanadi > preview.belgilanadi_royxat.length && <div className="pt-1" style={{ color: COLORS.muted }}>... va yana {preview.belgilanadi - preview.belgilanadi_royxat.length} ta</div>}</div></div>}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button onClick={() => send(true)} disabled={busy || !preview.belgilanadi}>{busy ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />} {preview.belgilanadi} ta talabani “Hujjat topshirgan” qilish</Button>
        <span className="text-xs" style={{ color: COLORS.muted }}>Allaqachon belgilanganlar o‘zgarmaydi. Xato bo‘lsa, ro‘yxatda “Hujjat topshirgan ✕” bosib bekor qilish mumkin.</span>
      </div>
    </div>}
    {done && <div className="mt-4 rounded-2xl p-4 text-sm" style={{ background: "#EAF7EF", color: "#28734B" }}>✅ {done.belgilandi} ta talaba “Hujjat topshirgan” qilib belgilandi{done.allaqachon ? ` · ${done.allaqachon} tasi allaqachon belgilangan edi` : ""}{done.topilmadi ? ` · ${done.topilmadi} ta ID topilmadi` : ""}.</div>}
  </Card>;
}
