  ...
  return <Card className="p-5">
    ...
    {s && <div className="mt-5 space-y-4">
      ...
      {type === "qabul" && matchingItems.length > 0 && <div className="space-y-3"><div><h4 className="font-black" style={{ color: COLORS.ink }}>Yo‘nalishlarni moslashtirish</h4><p className="text-xs" style={{ color: COLORS.muted }}>Tizim eng yaqinini oldindan tanladi. Har bir qatorni ko‘rib, kerak bo‘lsa almashtiring.</p></div>{matchingItems.map(([name, item]) => {
        const options = [...(item.yonalish_variantlari || []).map(x => ({ value: `p:${x.id}`, label: `${x.nomi} · ${x.kafedra_nomi} (${x.moslik_foizi}%)` })), ...(item.kafedra_variantlari || []).map(x => ({ value: `d:${x.id}`, label: `YANGI “${name}” → ${x.fakultet_nomi} · ${x.nomi} (${x.moslik_foizi}%)` }))];
        return <div key={name} className="rounded-2xl border p-4" style={{ borderColor: matches[name] ? "#B8DCC8" : "#EDBDBD", background: matches[name] ? "#F2FAF5" : "#FFF5F5" }}><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><div className="font-black" style={{ color: COLORS.ink }}>{item.yaratiladigan_yonalish_nomi || name}</div><div className="text-xs" style={{ color: COLORS.muted }}>{item.talaba_soni} talaba · {item.holat === "aniq" ? "mavjud yo‘nalish topildi" : item.holat === "yaqin_topildi" ? "yaqin nom avtomatik topildi" : item.holat === "yangi_yonalish" ? "shu kafedra ichida bitta yo‘nalish ochiladi" : "tasdiqlash kerak"}</div></div><Pill tone={matches[name] ? "green" : "red"}>{matches[name] ? "Tanlandi" : "Tanlanmagan"}</Pill></div><div className="mb-3 flex flex-wrap gap-1.5">{(item.variantlar || []).map(v => <Pill key={`${v.talim_shakli}-${v.talim_tili}`} tone="blue">{v.talim_shakli} · {v.talim_tili} — {v.talaba_soni}</Pill>)}</div><select className={inputClass} value={matches[name] || ""} onChange={e => setMatches(old => ({ ...old, [name]: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Yo‘nalish yoki kafedrani tanlang</option>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>;
      })}</div>}
