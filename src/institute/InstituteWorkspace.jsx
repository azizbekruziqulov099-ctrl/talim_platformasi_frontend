...
function Card({ children, className = "", style = {} }) {
  return <section className={`rounded-3xl border bg-white ${className}`} style={{ borderColor: COLORS.line, boxShadow: "0 12px 36px rgba(23,50,71,.06)", ...style }}>{children}</section>;
}
...
function FileImport({ apiBase, token, universityId, facultyId, type, templateHref, onCommitted, departments = [] }) {
  ...
  const commit = async () => {
    ...
    const missing = matchItems.filter(([name]) => !matches[name]).map(([name]) => name);
    if (type === "qabul" && missing.length) return setError(`Yo‘nalishni tanlang: ${missing.join(", ")}`);
    ...
    setBusy(true); setError("");
    try {
      const res = await fetch(`${apiBase}${commitPath}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, batch_id: preview.batch_id, yonalish_mosliklari: programMap, yangi_yonalish_kafedralari: departmentMap, otm_nomi_farqini_tasdiqlash: confirmOtm }) });
  ...
  const allMatched = type !== "qabul" || matchingItems.every(([name]) => matches[name]);
  return <Card className="p-5">
    ...
    {s && <div className="mt-5 space-y-4">
      ...
      {type === "qabul" && matchingItems.length > 0 && <div className="space-y-3"><div><h4 className="font-black" style={{ color: COLORS.ink }}>Yo‘nalishlarni moslashtirish</h4><p className="text-xs" style={{ color: COLORS.muted }}>Tizim eng yaqinini oldindan tanladi. Har bir qatorni ko‘rib, kerak bo‘lsa almashtiring.</p></div>{matchingItems.map(([name, item]) => {
        const options = (item.yonalish_variantlari || []).map(x => ({ value: `p:${x.id}`, label: `${x.nomi} · ${x.kafedra_nomi} (${x.moslik_foizi}%)` }));
        return <div key={name} className="rounded-2xl border p-4" style={{ borderColor: matches[name] ? "#B8DCC8" : "#EDBDBD", background: matches[name] ? "#F2FAF5" : "#FFF5F5" }}><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><div className="font-black" style={{ color: COLORS.ink }}>Excelda: {name}</div><div className="text-xs" style={{ color: COLORS.muted }}>{item.talaba_soni} talaba · tizim fakultetdagi mavjud yo‘nalishlardan eng mosini tanladi</div></div><Pill tone={matches[name] ? "green" : "red"}>{matches[name] ? "Moslandi" : "Tekshiring"}</Pill></div><div className="mb-3 flex flex-wrap gap-1.5">{(item.variantlar || []).map(v => <Pill key={`${v.talim_shakli}-${v.talim_tili}`} tone="blue">{v.talim_shakli} · {v.talim_tili} — {v.talaba_soni}</Pill>)}</div><select className={inputClass} value={matches[name] || ""} onChange={e => setMatches(old => ({ ...old, [name]: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Fakultetdagi mavjud yo‘nalishdan birini tanlang</option>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>;
      })}</div>}
...
function StructurePanel({ api, apiBase, token, universityId, canManage, onChanged, onCredentials, onNextAdmission, startMode, onStartModeConsumed }) {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap gap-2"><Button onClick={() => setMode("view")} kind={mode === "view" ? "primary" : "secondary"}>Tuzilma</Button>{canManage && <><Button onClick={() => setMode("manual")} kind={mode === "manual" ? "primary" : "secondary"}><Plus size={16} /> Qo‘lda kiritish</Button><Button onClick={() => setMode("import")} kind={mode === "import" ? "primary" : "secondary"}><FileSpreadsheet size={16} /> Shablon orqali import</Button></>}</div>
...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed }) {
  ...
  const [page, setPage] = useState(1); const [pages, setPages] = useState(0); const [detail, setDetail] = useState(null); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [selectedFacultyId, setSelectedFacultyId] = useState("");
  const faculties = structure?.fakultetlar || [];
...
function TutorPanel({ api, token, universityId, structure, canManage }) {
  ...
  const assign = async () => {
    setError("");
    try {
      await api("/api/institut/v20/tyutor/biriktir", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, universitet_id: universityId, tyutor_user_id: Number(form.tyutor_user_id), fakultet_id: form.fakultet_id ? Number(form.fakultet_id) : undefined, yonalish_id: form.yonalish_id ? Number(form.yonalish_id) : undefined, talim_shakli: form.talim_shakli || undefined, talim_tili: form.talim_tili || undefined }) });
      setForm({ tyutor_user_id: "", fakultet_id: "", yonalish_id: "", talim_shakli: "", talim_tili: "" }); load();
...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false }) {
  ...
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
      ...
      {tab === "dashboard" && <div className="space-y-4">{permissions.tuzilma_boshqarish && <Card className="p-5 md:p-6"><Pill tone="violet">TUZILMANI KIRITISH</Pill><h2 className="mt-3 text-xl font-black" style={{ color: COLORS.ink }}>Fakultet, kafedra va yo‘nalishlar</h2><p className="mb-5 mt-1 text-sm" style={{ color: COLORS.muted }}>Bu institut tuzilmasi. Talabalar alohida “1-kurs qabuli” bo‘limidan import qilinadi.</p><StructureEntryChoices onManual={() => openStructure("manual")} onImport={() => openStructure("import")} /></Card>}<Card className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><Pill tone="green">TALABA IMPORTI</Pill><h2 className="mt-2 text-xl font-black" style={{ color: COLORS.ink }}>Talabalarni import qilish</h2><p className="mt-1 text-sm" style={{ color: COLORS.muted }}>Qabul faylidagi talabalar yo‘nalish, ta’lim shakli va tiliga mos joylashtiriladi.</p></div><Button onClick={() => { setAdmissionStartMode("import"); setTab("admission"); }}><FileSpreadsheet size={16} /> Talaba faylini import qilish</Button></div></Card><Card className="p-5"><h2 className="text-xl font-black" style={{ color: COLORS.ink }}>Tizim tayyorligi</h2><div className="mt-4 grid gap-3 md:grid-cols-3">{[["Fakultet va kafedralar", bootstrap.sonlar.fakultet > 0], ["Ta’lim yo‘nalishlari", bootstrap.sonlar.yonalish > 0], ["Import qilingan talabalar", bootstrap.sonlar.talaba > 0]].map(([l, ok]) => <div key={l} className="rounded-2xl p-4" style={{ background: ok ? "#ECF7F0" : "#FFF6E8" }}><div className="flex items-center gap-2 font-black" style={{ color: ok ? COLORS.green : COLORS.amber }}>{ok ? <CheckCircle2 size={18} /> : <RefreshCcw size={18} />}{l}</div></div>)}</div><div className="mt-5 rounded-2xl border p-4 text-sm" style={{ borderColor: COLORS.line, color: COLORS.muted }}>Ketma-ketlik: <b>1) Institut tuzilmasi</b> → <b>2) Xodim va rollar</b> → <b>3) Talabalarni import qilish</b> → <b>4) Tyutorlarni biriktirish</b>.</div></Card></div>}
      {tab === "structure" && <StructurePanel api={api} apiBase={apiBase} token={token} universityId={id} canManage={permissions.tuzilma_boshqarish} onChanged={() => load()} onCredentials={setCredentials} onNextAdmission={() => { setAdmissionStartMode("import"); setTab("admission"); }} startMode={structureStartMode} onStartModeConsumed={() => setStructureStartMode(null)} />}
      ...
      {tab === "admission" && <AdmissionsPanel api={api} apiBase={apiBase} token={token} universityId={id} structure={structure} permissions={permissions} onCredentials={setCredentials} startMode={admissionStartMode} onStartModeConsumed={() => setAdmissionStartMode(null)} />}
