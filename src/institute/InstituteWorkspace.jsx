...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false, initialFacultyId = null }) {
  ...
  const heroStats = [[instituteFacultyCount, "Fakultet"], [canonicalProgramCount, "Yo‘nalish"], [teacherCount, "O‘qituvchi"], [bootstrap.sonlar.talaba, "Talaba"]];
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
      ...
      {tab === "dashboard" && <div className="space-y-4">{activeFaculty ? <>
        <Card className="p-5 md:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><Pill tone="blue">FAKULTET</Pill><h2 className="mt-2 text-xl font-black" style={{ color: COLORS.ink }}>{activeFaculty.nomi}</h2><p className="mt-1 text-sm" style={{ color: COLORS.muted }}>Fakultet haqida qisqa ma’lumot. Talaba ro‘yxati faqat yo‘nalish tanlangandan keyin ochiladi.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => navigateTab("structure")}><Building2 size={16} /> Yo‘nalishlarni ko‘rish</Button>{permissions.qabul_korish && <Button kind="secondary" onClick={() => { setAdmissionStartMode("list"); navigateTab("admission"); }}><ClipboardCheck size={16} /> Hujjat topshirish natijalari</Button>}</div></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[[activeFacultyStats.kafedra, "Kafedra"], [activeFacultyStats.yonalish, "Yo‘nalish"], [activeFacultyStats.oqituvchi, "O‘qituvchi"], [activeFacultyStats.talaba, "Talaba"]].map(([n, label]) => <div key={label} className="rounded-2xl p-4" style={{ background: COLORS.sky }}><div className="text-2xl font-black" style={{ color: COLORS.blue }}>{n}</div><div className="text-xs font-bold" style={{ color: COLORS.muted }}>{label}</div></div>)}</div></Card>
