...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null, lockedProgramIds = null, compactProgram = false }) {
  ...
  const revertStage = async (student, bosqich) => {
    const label = bosqich === 2 ? "Hujjat topshirgan" : "Bazaga kiritilgan";
    if (!window.confirm(`${student.fish}\n\n“${label}” belgisi xato qo‘yilganmi? OK bossangiz belgi bekor qilinadi va talaba yana “${bosqich === 2 ? "Hujjat topshirmagan" : "Bazaga kiritilmagan"}” holatiga qaytadi.`)) return;
    try {
      ...
      setItems(old => old.map(item => String(item.id) !== String(student.id) ? item : {
        ...item,
