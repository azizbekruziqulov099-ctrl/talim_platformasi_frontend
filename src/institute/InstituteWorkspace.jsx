...
function FileImport({ apiBase, token, universityId, facultyId, type, templateHref, onCommitted, departments = [] }) {
  ...
  return <Card className="p-5">
    ...
    {s && <div className="mt-5 space-y-4">
      ...
      {isAdmission && matchingItems.length > 0 && <div className="space-y-3"><div><h4 className="font-black" style={{ color: COLORS.ink }}>Yo‘nalishlarni mavjud tuzilmaga moslash</h4><p className="text-xs" style={{ color: COLORS.muted }}>Har bir Excel yo‘nalishini shu fakultetdagi mavjud yo‘nalishga ulang. Import yangi kafedra yoki yo‘nalish yaratmaydi.</p></div>{matchingItems.map(([name, item]) => {
        const matched = !!matches[name];
        return <div key={name} className="rounded-2xl border p-4" style={{ borderColor: matched ? "#B8DCC8" : "#EDBDBD", background: matched ? "#F2FAF5" : "#FFF6F6" }}><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><div className="font-black" style={{ color: COLORS.ink }}>Excelda: {name}</div><div className="text-xs" style={{ color: COLORS.muted }}>{item.talaba_soni} talaba · {matched ? "mavjud yo‘nalishga bog‘lanadi" : "mos yo‘nalishni tanlash majburiy"}</div></div><Pill tone={matched ? "green" : "red"}>{matched ? "Moslandi" : "Tanlanmagan"}</Pill></div><div className="mb-3 flex flex-wrap gap-1.5">{(item.variantlar || []).map(v => <Pill key={`${v.talim_shakli}-${v.talim_tili}`} tone="blue">{v.talim_shakli} · {v.talim_tili} — {v.talaba_soni}</Pill>)}</div><select className={inputClass} value={matches[name] || ""} onChange={e => setMatches(old => ({ ...old, [name]: e.target.value }))} style={{ borderColor: matched ? COLORS.line : "#EDBDBD" }}><option value="">Mavjud yo‘nalishni tanlang</option>{availablePrograms.map(program => <option key={program.id} value={`p:${program.id}`}>{program.kafedra_nomi} · {program.nomi}</option>)}</select></div>;
      })}</div>}
      ...
      {isAdmission && !allMatched && <ErrorBox text="Import qilishdan oldin Excelda uchragan barcha yo‘nalishlarni mavjud yo‘nalishlarga moslang." />}
      <Button onClick={commit} disabled={busy || !preview.commit_mumkin || !allMatched || (isAdmission && !!s.otm_nomi_farqi?.length && !confirmOtm)} className="w-full"><CheckCircle2 size={17} /> {isAdmission ? "Moslangan talabalarni import qilish" : isStaff ? "Xodimlarni va kirish kodlarini import qilish" : "Xatosiz ma’lumotlarni bazaga kiritish"}</Button>
    </div>}
...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, onOpenStaff, onBackToWorkspace, onInitialFacultyBack, onProgramDepthChange, onFacultyContextChange, startMode, onStartModeConsumed, staffRows = [], focusFacultyId = null, facultyContextLocked = false }) {
  ...
  if (selectedFaculty && selectedDepartment) return <div className="space-y-4">
    <button onClick={() => { setSelectedProgram(null); setSelectedDepartment(null); }} className="inline-flex items-center gap-2 text-sm font-black" style={{ color: COLORS.blue }}><ArrowLeft size={17} /> {selectedFaculty.nomi} kafedralariga qaytish</button>
    ...
    <Card className="p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Pill tone="blue">{(selectedDepartment.yonalishlar || []).length} ta</Pill>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {(selectedDepartment.yonalishlar || []).map((y, index) => <div key={y.id} className="rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md" style={{ borderColor: COLORS.line, background: "#fff" }}>
          <div className="flex items-start justify-between gap-3">
            <button type="button" onClick={() => setSelectedProgram({ ...y, kafedra_nomi: selectedDepartment.nomi, kafedra_id: selectedDepartment.id })} className="min-w-0 flex-1 text-left">
              ...
              <div className="mt-1 text-xs" style={{ color: COLORS.muted }}>{y.daraja || "Daraja belgilanmagan"} · {selectedDepartment.nomi}</div>
            </button>
        ...
        {!(selectedDepartment.yonalishlar || []).length && <Empty>Bu kafedrada hali yo‘nalish yo‘q.</Empty>}
      </div>
    </Card>
  </div>;
  ...
  if (selectedFaculty) return <div className="space-y-4">
    ...
    <Card className="p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div><Pill tone="green">KAFEDRALAR</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>Kafedrani tanlang — talabalar, o‘qituvchilar va tyutorlar shu yerda</h3><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>Har kafedra o‘z yo‘nalishi(lari) bilan. Kafedra bitta yo‘nalishli bo‘lsa, bosganda to‘g‘ridan-to‘g‘ri talabalar ochiladi.</p></div>
        <div className="flex items-center gap-2"><Pill tone="green">{(selectedFaculty.kafedralar || []).length} kafedra</Pill><Pill tone="blue">{facultyPrograms.length} yo‘nalish</Pill>{canManage && <Button kind="secondary" onClick={() => setMode("manual")}><Plus size={16} /> Kafedra / yo‘nalish qo‘shish</Button>}</div>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {(selectedFaculty.kafedralar || []).map(department => {
          ...
          return <div key={department.id} className="rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md" style={{ borderColor: COLORS.line, background: "#fff" }}>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                {canManage && <button type="button" onClick={() => archiveStructure("kafedra", department)} className="rounded-lg border px-2 py-1 font-black" style={{ borderColor: COLORS.line }} title="Kafedra amallari">…</button>}
                <button type="button" onClick={openDepartment}><ChevronRight size={18} style={{ color: COLORS.green }} /></button>
              </div>
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2">{[[students, "Talaba"], [teachers, "O‘qituvchi"], [tutors, "Tyutor"]].map(([n, label]) => <div key={label} className="rounded-xl p-2 text-center" style={{ background: COLORS.cream }}><div className="text-lg font-black" style={{ color: COLORS.ink }}>{n}</div><div className="text-[10px] font-bold" style={{ color: COLORS.muted }}>{label}</div></div>)}</div>
            <div className="mt-3 space-y-2">
              {programs.map(y => <button type="button" key={y.id} onClick={() => openProgramFromFaculty({ ...y, kafedra_nomi: department.nomi, kafedra_id: department.id })} className="flex w-full items-start justify-between gap-2 rounded-xl border px-3 py-2 text-left hover:bg-slate-50" style={{ borderColor: COLORS.line }}>
                <span className="min-w-0"><span className="block text-sm font-black" style={{ color: COLORS.blue }}>{y.nomi}</span><span className="mt-0.5 flex flex-wrap gap-1"><Pill tone={Number(y.talaba_soni) ? "green" : "gray"}>{y.talaba_soni || 0} talaba</Pill>{y.daraja && <Pill tone="gray">{y.daraja}</Pill>}{(y.talim_shakllari || []).map(x => <Pill key={`${y.id}-s-${x}`} tone="blue">{x}</Pill>)}{(y.talim_tillari || []).map(x => <Pill key={`${y.id}-t-${x}`} tone="violet">{x}</Pill>)}</span></span>
                <ChevronRight size={16} className="mt-1 shrink-0" style={{ color: COLORS.blue }} />
              </button>)}
              {!programs.length && <div className="rounded-xl border border-dashed px-3 py-2 text-xs" style={{ borderColor: COLORS.line, color: COLORS.muted }}>Bu kafedrada hali yo‘nalish yo‘q{canManage ? " — “+ Yo‘nalish” bilan qo‘shing" : ""}.</div>}
            </div>
          </div>;
        })}
        {!(selectedFaculty.kafedralar || []).length && <Empty>Bu fakultetda hali kafedra yo‘q.</Empty>}
      </div>
    </Card>
  </div>;
  return <div className="space-y-4">
    ...
    {mode === "view" && <>{!data.fakultetlar.length ? <Card className="p-5 md:p-6"><Pill tone="amber">BIRINCHI QADAM</Pill><h2 className="mt-3 text-xl font-black" style={{ color: COLORS.ink }}>Institut tuzilmasini qanday kiritasiz?</h2><p className="mb-5 mt-1 text-sm" style={{ color: COLORS.muted }}>Ikkala usul ham bir xil natija beradi. Istalganini tanlang.</p>{canManage ? <StructureEntryChoices onManual={() => setMode("manual")} onImport={() => setMode("import")} /> : <Empty>Tuzilmani kiritish uchun administrator ruxsati kerak.</Empty>}</Card> : data.fakultetlar.map((f, facultyIndex) => <Card key={f.id} onClick={() => { setSelectedFaculty(f); onFacultyContextChange?.(f.id); }} className="cursor-pointer overflow-hidden transition hover:-translate-y-0.5 hover:shadow-lg"><div className="flex flex-wrap items-center justify-between gap-3 p-5" style={{ background: "linear-gradient(135deg,#EDF7F8,#FBF7EE)" }}><div><h3 className="text-lg font-black" style={{ color: COLORS.ink }}>{facultyIndex + 1}. {f.nomi}</h3><p className="text-xs" style={{ color: COLORS.muted }}>{f.kafedra_soni} kafedra · {f.yonalish_soni} yo‘nalish</p><div className="mt-2 flex flex-wrap gap-1">{(f.rahbariyat || []).filter(x => x.rol !== "fakultet_admin").map(x => <Pill key={x.id} tone="violet">{ROLE_OPTIONS.find(r => r[0] === x.rol)?.[1] || x.rol}: {x.full_name}</Pill>)}</div></div><Pill tone={f.toldirilish.tayyor ? "green" : "amber"}>{f.toldirilish.tayyor ? "Rahbariyat to‘liq" : `Dekan ${f.toldirilish.dekan}/1 · Zam ${f.toldirilish.zam_dekan}/2 · Ma’naviy ${f.toldirilish.manaviyatchi}/1 · Admin ${f.toldirilish.admin}/1`}</Pill></div><div className="grid gap-3 p-5 md:grid-cols-2">{f.kafedralar.map(d => <div key={d.id} className="rounded-2xl border p-4" style={{ borderColor: COLORS.line }}><div className="flex items-center justify-between gap-2"><div className="font-black" style={{ color: COLORS.teal }}>{d.nomi}</div>{canManage && <button onClick={e => { e.stopPropagation(); archiveStructure("kafedra", d); }} className="rounded-lg border px-2 py-1 font-black" style={{ borderColor: COLORS.line }} title="Kafedra amallari">…</button>}</div><div className="mt-1 text-xs font-bold" style={{ color: d.mudir ? COLORS.green : COLORS.red }}>{d.mudir ? `Mudir: ${d.mudir.full_name}` : "Mudir kiritilmagan"}</div><div className="mt-2 space-y-1">{d.yonalishlar.length ? d.yonalishlar.map((y, directionIndex) => <div key={y.id} className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-xs" style={{ background: COLORS.cream, color: COLORS.ink }}><span>{directionIndex + 1}. {y.nomi} · {y.daraja}</span>{canManage && <button onClick={e => { e.stopPropagation(); archiveStructure("yonalish", y); }} className="font-black" title="Yo‘nalish amallari">…</button>}</div>) : <span className="text-xs" style={{ color: COLORS.muted }}>Yo‘nalish yo‘q</span>}</div></div>)}</div></Card>)}</>}
    <ErrorBox text={error} />
  </div>;
}
...
function AdmissionTypePill({ value }) {
  ...
  if (normalized.includes("grant") || normalized.includes("budjet")) return <Pill tone="green">Budjet / grant</Pill>;
  if (normalized.includes("kontrakt") || normalized.includes("shartnoma")) return <Pill tone="violet">To‘lov-kontrakt</Pill>;
  return <Pill tone="gray">{value || "Belgilanmagan"}</Pill>;
}
...
function StudentContactActions({ student }) {
  ...
  return <div className="px-5 pb-5">
    <Field label="Abituriyentning ta’lim tilidagi SMS matni" hint="Matnni xohlasangiz tahrirlang, keyin SMS tugmasini bosing.">
    ...
    <div className="mt-3 flex flex-wrap gap-2"><a href={`tel:${phone}`}><Button kind="secondary"><Phone size={15} /> Bir bosishda qo‘ng‘iroq</Button></a><a href={`sms:${phone}?body=${encodeURIComponent(message)}`}><Button><Send size={15} /> SMS ilovasiga yuborish</Button></a><a href={`https://wa.me/${digits}`} target="_blank" rel="noreferrer"><Button kind="secondary"><MessageCircle size={15} /> WhatsApp</Button></a><a href={student.telegram_username ? `https://t.me/${student.telegram_username.replace(/^@/, "")}` : `tg://resolve?phone=${digits}`}><Button kind="secondary">Telegram</Button></a>{student.max_username ? <a href={`https://max.ru/${student.max_username.replace(/^@/, "")}`} target="_blank" rel="noreferrer"><Button kind="secondary">MAX</Button></a> : <Button kind="secondary" disabled>MAX ulanmagan</Button>}</div>
  </div>;
}
...
function AdmissionRowsExport({ apiBase, params, activeStatus = "all", disabled = false, className = "" }) {
  ...
  return <div className={`flex min-w-0 flex-wrap items-end gap-2 ${className}`}>
    <label className="min-w-0 flex-1 text-xs font-bold" style={{ color: COLORS.muted }}>
      Excelga yuklanadigan ro‘yxat
      <select className={`${inputClass} mt-1`} value={status} disabled={disabled} onChange={event => setStatus(event.target.value)} style={{ borderColor: COLORS.line }}>
        <option value="all">Barchasi</option>
        <option value="hujjat">Hujjat topshirganlar</option>
        <option value="topshirmagan">Hujjat topshirmaganlar</option>
      </select>
    </label>
    {disabled ? <button type="button" disabled className="rounded-xl px-4 py-2.5 text-sm font-black text-white opacity-50" style={{ background: COLORS.green }}>Excel yuklash</button>
      : <a href={admissionRowsExportHref(apiBase, params, status)} className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black text-white" style={{ background: COLORS.green }}><FileSpreadsheet size={16} /> Excel yuklash</a>}
  </div>;
}
...
function AdmissionReports({ api, apiBase, token, universityId, structure, facultyId = null }) {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap gap-2"><Button onClick={() => setSection("daily")} kind={section === "daily" ? "primary" : "secondary"}>Kunlik qabul hisoboti</Button><Button onClick={() => setSection("general")} kind={section === "general" ? "primary" : "secondary"}>Umumiy hisobot</Button></div>
    {section === "daily" && <Card className="p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><Pill tone="blue">KUNLIK QABUL</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>Yo‘nalish va kunlar kesimi</h3><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>Har bir katak — shu kuni hujjat topshirganlar. Jami server hisobidan olinadi.</p></div><div className="flex flex-wrap items-end gap-2"><Field label="Boshlanish"><input type="date" className={inputClass} value={range.boshlanish} onChange={e => setRange(old => ({ ...old, boshlanish: e.target.value }))} style={{ borderColor: COLORS.line }} /></Field><Field label="Tugash"><input type="date" className={inputClass} value={range.tugash} onChange={e => setRange(old => ({ ...old, tugash: e.target.value }))} style={{ borderColor: COLORS.line }} /></Field><Button onClick={loadDaily} disabled={dailyBusy}>{dailyBusy ? <Loader2 size={16} className="animate-spin" /> : <RefreshCcw size={16} />} Ko‘rsatish</Button><a href={exportLink("/api/institut/v20/qabul/kunlik_hisobot.xlsx", dailyParams)} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-black" style={{ borderColor: COLORS.line, color: COLORS.blue }}><Download size={16} /> Excel</a></div></div><div className="mt-4"><ErrorBox text={dailyError} /></div>{!dailyBusy && !!days.length && <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead style={{ background: COLORS.cream, color: COLORS.muted }}><tr><th className="px-3 py-2">№</th><th className="px-3 py-2">Yo‘nalish</th>{days.map(day => <th key={day} className="px-3 py-2 text-center">{day.slice(5).split("-").reverse().join(".")}</th>)}<th className="px-3 py-2 text-center">Jami</th></tr></thead><tbody>{dailyRows.map((row, index) => <tr key={`${row.fakultet_id || "f"}-${row.kafedra_id || "k"}-${row.yonalish_id || index}-${row.talim_shakli || "all"}-${row.talim_tili || "all"}`} className="border-t" style={{ borderColor: COLORS.line }}><td className="px-3 py-2">{index + 1}</td><td className="px-3 py-2"><div className="font-black">{row.yonalish_nomi || row.nomi}</div><div className="text-[11px]" style={{ color: COLORS.muted }}>{row.kafedra_nomi || row.fakultet_nomi}</div></td>{days.map(day => <td key={day} className="px-3 py-2 text-center">{dailyCell(row, day)}</td>)}<td className="px-3 py-2 text-center font-black">{Number(row.jami ?? days.reduce((sum, day) => sum + dailyCell(row, day), 0))}</td></tr>)}</tbody><tfoot><tr className="border-t-2 font-black" style={{ borderColor: COLORS.line }}><td className="px-3 py-3" colSpan="2">Jami</td>{days.map(day => <td key={day} className="px-3 py-3 text-center">{dailyColumnTotal(day)}</td>)}<td className="px-3 py-3 text-center">{dailyGrandTotal}</td></tr></tfoot></table></div>}{!dailyBusy && daily && !dailyRows.length && <div className="mt-4"><Empty>Tanlangan kunlarda qabul harakati yo‘q.</Empty></div>}</Card>}
    {section === "general" && <><Card className="p-4"><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4"><div className="relative"><Search size={16} className="absolute left-3 top-3" style={{ color: COLORS.muted }} /><input className={`${inputClass} pl-9`} value={filters.q} onChange={e => setFilters(old => ({ ...old, q: e.target.value }))} placeholder="F.I.Sh. yoki AbiturID" style={{ borderColor: COLORS.line }} /></div>{!facultyId && <select className={inputClass} value={filters.fakultet_id} onChange={e => setFilters(old => ({ ...old, fakultet_id: e.target.value, kafedra_id: "", yonalish_id: "" }))} style={{ borderColor: COLORS.line }}><option value="">Barcha fakultet</option>{faculties.map(faculty => <option key={faculty.id} value={faculty.id}>{faculty.nomi}</option>)}</select>}<select className={inputClass} value={filters.kafedra_id} onChange={e => setFilters(old => ({ ...old, kafedra_id: e.target.value, yonalish_id: "" }))} style={{ borderColor: COLORS.line }}><option value="">Barcha kafedra</option>{visibleDepartments.map(department => <option key={department.id} value={department.id}>{department.nomi}</option>)}</select><select className={inputClass} value={filters.yonalish_id} onChange={e => setFilters(old => ({ ...old, yonalish_id: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Barcha yo‘nalish</option>{visiblePrograms.map(program => <option key={program.id} value={program.id}>{program.nomi}</option>)}</select><select className={inputClass} value={filters.talim_shakli} onChange={e => setFilters(old => ({ ...old, talim_shakli: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Barcha ta’lim shakli</option>{(general?.filtrlar?.shakllar || []).map(value => <option key={value}>{value}</option>)}</select><select className={inputClass} value={filters.talim_tili} onChange={e => setFilters(old => ({ ...old, talim_tili: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Barcha ta’lim tili</option>{(general?.filtrlar?.tillar || []).map(value => <option key={value}>{value}</option>)}</select><select className={inputClass} value={filters.qabul_turi} onChange={e => setFilters(old => ({ ...old, qabul_turi: e.target.value }))} style={{ borderColor: COLORS.line }}><option value="">Budjet + kontrakt</option><option value="grant">Budjet / grant</option><option value="kontrakt">Kontrakt</option></select><a href={exportLink("/api/institut/v20/qabul/umumiy_hisobot.xlsx", generalParams)} className="inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-black" style={{ borderColor: COLORS.line, color: COLORS.blue }}><Download size={16} /> Excel hisobot</a><AdmissionRowsExport key={universityId} apiBase={apiBase} params={generalExportParams} className="md:col-span-2" /></div></Card><ErrorBox text={generalError} />{generalBusy && <Card className="p-10 text-center"><Loader2 className="mx-auto animate-spin" style={{ color: COLORS.blue }} /></Card>}{!generalBusy && general && <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{[["Jami", totals.jami || 0, "blue", "all"], ["Hujjat topshirgan", totals.hujjat || 0, "amber", "hujjat"], ["Hujjat qolgan", totals.qolgan ?? Math.max(0, Number(totals.jami || 0) - Number(totals.hujjat || 0)), "red", "topshirmagan"], ["HEMISga kiritilgan", totals.hemis ?? totals.baza ?? 0, "violet", "baza"], ["HEMIS kutilmoqda", totals.hemis_kutilmoqda ?? Math.max(0, Number(totals.hujjat || 0) - Number(totals.hemis ?? totals.baza ?? 0)), "gray", "hemis_kutilmoqda"]].map(([label, value, tone, holat]) => { const active = drill && drill.holat === holat && !drill.yonalish_id; return <button type="button" key={label} onClick={() => active ? setDrill(null) : openDrill(holat, label)} className="rounded-2xl border bg-white p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md" style={{ borderColor: active ? COLORS.blue : COLORS.line, boxShadow: active ? `0 0 0 2px ${COLORS.sky}` : undefined }}><Pill tone={tone}>{label}</Pill><div className="mt-2 text-2xl font-black" style={{ color: COLORS.ink }}>{value}</div><div className="mt-1 text-[11px]" style={{ color: COLORS.muted }}>{active ? "Ro‘yxat pastda · yopish" : "Kimlar — ko‘rish"}</div></button>; })}</div>{!!generalRows.length && <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead style={{ background: COLORS.cream, color: COLORS.muted }}><tr>{["Kafedra", "Yo‘nalish", "Jami", "Hujjat", "Qolgan", "HEMIS"].map(label => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{generalRows.map((row, index) => <tr key={row.yonalish_id || index} className="border-t" style={{ borderColor: COLORS.line }}><td className="px-4 py-3">{row.kafedra_nomi || "—"}</td><td className="px-4 py-3 font-black">{row.yonalish_nomi || row.nomi}</td>{[["all", row.jami || 0, "Jami"], ["hujjat", row.hujjat || 0, "Hujjat topshirgan"], ["topshirmagan", row.qolgan ?? Math.max(0, Number(row.jami || 0) - Number(row.hujjat || 0)), "Hujjat qolgan"], ["baza", row.hemis ?? row.baza ?? 0, "HEMISga kiritilgan"]].map(([holat, value, label]) => { const active = drill && drill.holat === holat && String(drill.yonalish_id || "") === String(row.yonalish_id || ""); return <td key={holat} className="px-2 py-1"><button type="button" disabled={!row.yonalish_id} onClick={() => active ? setDrill(null) : openDrill(holat, `${row.yonalish_nomi || row.nomi} · ${label}`, { yonalish_id: row.yonalish_id })} className="w-full rounded-lg px-2 py-2 text-left font-bold transition hover:bg-slate-50 disabled:cursor-default" style={{ color: Number(value) ? COLORS.blue : COLORS.muted, background: active ? COLORS.sky : undefined, textDecoration: Number(value) && row.yonalish_id ? "underline dotted" : "none" }} title="Kimlar — ko‘rish">{value}</button></td>; })}</tr>)}</tbody></table></div></Card>}
    {drill && <Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b p-4" style={{ borderColor: COLORS.line, background: COLORS.cream }}><div><Pill tone="blue">RO‘YXAT</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>{drill.label}</h3><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>{drillTotal} ta talaba · yuqoridagi til, ta’lim shakli va to‘lov filtrlari qo‘llangan</p></div><div className="flex items-center gap-2">{drillPages > 1 && <><Button kind="secondary" disabled={drillPage <= 1 || drillBusy} onClick={() => setDrillPage(p => p - 1)}>‹</Button><span className="text-xs font-bold" style={{ color: COLORS.muted }}>{drillPage} / {drillPages}</span><Button kind="secondary" disabled={drillPage >= drillPages || drillBusy} onClick={() => setDrillPage(p => p + 1)}>›</Button></>}<Button kind="secondary" onClick={() => setDrill(null)}>Yopish</Button></div></div><ErrorBox text={drillError} />{drillBusy ? <div className="p-10 text-center"><Loader2 className="mx-auto animate-spin" style={{ color: COLORS.blue }} /></div> : <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead style={{ background: COLORS.cream, color: COLORS.muted }}><tr>{["№", "F.I.Sh.", "Yo‘nalish", "Shakl", "Til", "To‘lov", "Ball", "Telefon", "Holat"].map(label => <th key={label} className="px-3 py-2">{label}</th>)}</tr></thead><tbody>{drillItems.map((item, index) => <tr key={item.id} className="border-t" style={{ borderColor: COLORS.line }}><td className="px-3 py-2">{(drillPage - 1) * 50 + index + 1}</td><td className="px-3 py-2 font-black" style={{ color: COLORS.ink }}>{item.fish}</td><td className="px-3 py-2">{item.yonalish_nomi}</td><td className="px-3 py-2">{item.talim_shakli}</td><td className="px-3 py-2">{item.talim_tili}</td><td className="px-3 py-2">{item.tavsiya_turi || "—"}</td><td className="px-3 py-2">{item.ball ?? "—"}</td><td className="px-3 py-2">{item.telefon_mask || "—"}</td><td className="px-3 py-2"><div className="flex flex-wrap gap-1">{item.hujjat_topshirgan ? <Pill tone="amber">Hujjat topshirgan</Pill> : <Pill tone="red">Hujjat qolgan</Pill>}{item.bazaga_kiritilgan ? <Pill tone="violet">HEMIS</Pill> : null}{item.saytga_kirgan ? <Pill tone="green">Saytga kirgan</Pill> : null}</div></td></tr>)}{!drillItems.length && <tr><td colSpan={9} className="px-3 py-6 text-center" style={{ color: COLORS.muted }}>Bu filtrda talaba yo‘q</td></tr>}</tbody></table></div>}</Card>}</>}</>}
  </div>;
}
...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null, lockedProgramIds = null, compactProgram = false }) {
  ...
  return <div className="space-y-4">
    {!compactProgram && <div className="flex flex-wrap gap-2"><Button onClick={() => setMode("list")} kind={mode === "list" ? "primary" : "secondary"}><Users size={16} /> Talabalar</Button>{canViewReports && <Button onClick={() => setMode("reports")} kind={mode === "reports" ? "primary" : "secondary"}><ClipboardCheck size={16} /> Hujjat qabuli hisoboti</Button>}{permissions.hujjat_belgilash && !lockedProgramId && <Button onClick={() => setMode("import")} kind={mode === "import" ? "primary" : "secondary"}><FileSpreadsheet size={16} /> Talabalarni import qilish</Button>}{permissions.hujjat_belgilash && <Button onClick={() => setMode("hujjat_import")} kind={mode === "hujjat_import" ? "primary" : "secondary"}><CheckCircle2 size={16} /> Excel orqali belgilash</Button>}</div>}
    ...
    {(compactProgram || mode === "list") && <>
      {!compactProgram && (fullStatuses ? <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{statusCards.map(([id, label, count, tone]) => <button key={id} onClick={() => { setTab(id); setPage(1); }} className="rounded-2xl border p-4 text-left" style={{ borderColor: tab === id ? COLORS.blue : COLORS.line, background: tab === id ? COLORS.sky : "#fff" }}><Pill tone={tone}>{id === "all" ? "UMUMIY" : id === "suhbat" ? "TELEFON SUHBATI" : "HUJJAT HOLATI"}</Pill><div className="mt-2 text-2xl font-black" style={{ color: COLORS.ink }}>{count}</div><div className="text-xs font-bold" style={{ color: COLORS.muted }}>{label}</div></button>)}</div><div className="grid gap-3 sm:grid-cols-2"><button onClick={() => { setFilters(old => ({ ...old, qabul_turi: old.qabul_turi === "grant" ? "" : "grant" })); setPage(1); }} className="rounded-2xl border bg-white p-4 text-left" style={{ borderColor: filters.qabul_turi === "grant" ? COLORS.green : COLORS.line }}><Pill tone="green">DAVLAT GRANTI</Pill><div className="mt-2 text-2xl font-black" style={{ color: COLORS.ink }}>{counts.grant || 0}</div></button><button onClick={() => { setFilters(old => ({ ...old, qabul_turi: old.qabul_turi === "kontrakt" ? "" : "kontrakt" })); setPage(1); }} className="rounded-2xl border bg-white p-4 text-left" style={{ borderColor: filters.qabul_turi === "kontrakt" ? COLORS.violet : COLORS.line }}><Pill tone="violet">TO‘LOV-KONTRAKT</Pill><div className="mt-2 text-2xl font-black" style={{ color: COLORS.ink }}>{counts.kontrakt || 0}</div></button></div></> : <Card className="p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><Pill tone="violet">HEMIS QAMROVI</Pill><div className="mt-2 font-black" style={{ color: COLORS.ink }}>Siz faqat biriktirilgan kafedra yoki yo‘nalish talabalarini ko‘rasiz</div><div className="mt-1 text-xs" style={{ color: COLORS.muted }}>Kafedra mudiri va tyutor o‘z qamrovida “Bazaga kiritildi” hamda “Saytga kirdi” holatini belgilaydi.</div></div><div className="flex gap-5 text-right"><div><div className="text-2xl font-black" style={{ color: COLORS.violet }}>{counts.baza || 0}</div><div className="text-xs font-bold" style={{ color: COLORS.muted }}>bazaga kiritilgan</div></div><div><div className="text-2xl font-black" style={{ color: COLORS.green }}>{counts.sayt || 0}</div><div className="text-xs font-bold" style={{ color: COLORS.muted }}>saytga kirgan</div></div></div></div></Card>)}
      <Card className="p-4"><div className={`grid gap-2 md:grid-cols-2 ${compactProgram ? "xl:grid-cols-3" : "xl:grid-cols-4"}`}><div className="relative xl:col-span-2"><Search size={16} className="absolute left-3 top-3" style={{ color: COLORS.muted }} /><input className={`${inputClass} pl-9`} value={filters.q} onChange={e => { setFilters({ ...filters, q: e.target.value }); setPage(1); }} placeholder="F.I.Sh. yoki AbiturID" style={{ borderColor: COLORS.line }} /></div>{!compactProgram && !lockedFacultyId && <select className={inputClass} value={selectedFacultyId} onChange={e => { setSelectedFacultyId(e.target.value); setFilters(old => ({ ...old, yonalish_id: "" })); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Barcha fakultet</option>{faculties.map(f => <option key={f.id} value={f.id}>{f.nomi}</option>)}</select>}{!compactProgram && <select className={inputClass} value={filters.yonalish_id} onChange={e => { setFilters({ ...filters, yonalish_id: e.target.value, talim_shakli: "", talim_tili: "", region: "" }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Barcha yo‘nalish</option>{visiblePrograms.map(y => <option key={y.id} value={y.id}>{y.nomi}</option>)}</select>}<select className={inputClass} value={filters.talim_shakli} onChange={e => { setFilters({ ...filters, talim_shakli: e.target.value }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Barcha shakl</option>{(filterOptions.shakllar || []).map(x => <option key={x}>{x}</option>)}</select><select className={inputClass} value={filters.talim_tili} onChange={e => { setFilters({ ...filters, talim_tili: e.target.value }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Barcha til</option>{(filterOptions.tillar || []).map(x => <option key={x}>{x}</option>)}</select><select className={inputClass} value={filters.qabul_turi} onChange={e => { setFilters({ ...filters, qabul_turi: e.target.value }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Grant + kontrakt</option><option value="grant">Davlat granti</option><option value="kontrakt">To‘lov-kontrakt</option></select><select className={inputClass} value={filters.region} onChange={e => { setFilters({ ...filters, region: e.target.value }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="">Barcha hudud</option>{(filterOptions.hududlar || []).map(x => <option key={x}>{x}</option>)}</select></div><div className="mt-3 flex flex-wrap items-end justify-between gap-3"><select className="rounded-lg border px-2 py-1.5 text-xs" value={filters.sort} onChange={e => { setFilters({ ...filters, sort: e.target.value }); setPage(1); }} style={{ borderColor: COLORS.line }}><option value="ball_desc">Ball: yuqoridan</option><option value="ball_asc">Ball: pastdan</option><option value="name">F.I.Sh.</option><option value="newest">Yangi import</option></select><button onClick={() => load()} className="inline-flex items-center gap-1 text-xs font-black" style={{ color: COLORS.blue }}><RefreshCcw size={14} /> Yangilash</button>{permissions.hujjat_belgilash && <AdmissionRowsExport key={universityId} apiBase={apiBase} params={listFilter.params} activeStatus={tab} disabled={!!listFilter.scopeError} />}</div></Card>
      <ErrorBox text={error} />
      <Card className="overflow-hidden">{busy ? <div className="p-12 text-center"><Loader2 className="mx-auto animate-spin" style={{ color: COLORS.blue }} /></div> : !items.length ? <div className="p-5"><Empty>Bu filtrda talaba yo‘q</Empty></div> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm" style={{ tableLayout: "fixed" }}><colgroup><col style={{ width: 40 }} /><col style={{ width: "26%" }} /><col style={{ width: "22%" }} /><col style={{ width: "16%" }} /><col /></colgroup><thead style={{ background: COLORS.cream, color: COLORS.muted }}><tr>{["№", "F.I.Sh. · ball · telefon", "Yo‘nalish · shakl · til", "Qabul turi · qayerdan", "Holat va amal"].map(h => <th key={h} className="px-3 py-2.5 text-[11px] font-black">{h}</th>)}</tr></thead><tbody>{items.map((s, index) => <tr key={s.id} className="border-t align-top" style={{ borderColor: COLORS.line }}><td className="px-3 py-3 text-xs font-black" style={{ color: COLORS.muted }}>{s.tartib_raqami || (page - 1) * 50 + index + 1}</td><td className="px-3 py-3"><button onClick={() => showDetail(s.id)} className="text-left text-sm font-black leading-snug hover:underline" style={{ color: COLORS.ink, overflowWrap: "anywhere" }}>{s.fish}</button><div className="mt-1 flex flex-wrap items-center gap-2 text-xs"><span className="font-black" style={{ color: COLORS.blue }}>Ball: {s.ball ?? "—"}</span><button onClick={() => showDetail(s.id)} className="inline-flex items-center gap-1 rounded-lg border px-2 py-0.5 text-[11px] font-black" style={{ borderColor: COLORS.line, color: COLORS.blue }}><Phone size={12} /> {s.telefon || s.telefon_mask || "Raqam"}</button></div><button type="button" onClick={() => saveNote(s)} className="mt-1.5 block w-full rounded-lg border px-2 py-1 text-left text-[11px] leading-snug" style={s.aloqa_izohi ? { borderColor: "#BFE3CC", background: "#F1FAF4", color: COLORS.green } : { borderColor: COLORS.line, color: COLORS.muted, borderStyle: "dashed" }} title={s.aloqa_izohi ? `Izoh (${s.aloqa_izohi_at ? new Date(s.aloqa_izohi_at).toLocaleDateString("uz") : ""}) — o‘zgartirish uchun bosing` : "Telefon suhbati izohini yozish"}>{s.aloqa_izohi ? <><span className="font-black">📝 </span>{s.aloqa_izohi}</> : "✎ Izoh yozish (suhbat natijasi)"}</button></td><td className="px-3 py-3"><div className="text-sm font-bold leading-snug" style={{ color: COLORS.ink }}>{s.yonalish_nomi}</div><div className="mt-1 text-xs" style={{ color: COLORS.muted }}>{[s.talim_shakli, s.talim_tili].filter(Boolean).join(" · ") || "—"}</div></td><td className="px-3 py-3"><AdmissionTypePill value={s.tavsiya_turi} /><div className="mt-1 text-xs leading-snug" style={{ color: COLORS.muted }}>{[s.doimiy_region, s.doimiy_tuman].filter(Boolean).join(", ") || "—"}</div></td><td className="px-3 py-3"><div className="flex flex-wrap gap-1">{fullStatuses ? <><Pill tone="blue">Talaba</Pill>{s.hujjat_topshirgan && !s.bazaga_kiritilgan && permissions.hujjat_belgilash ? <button type="button" onClick={() => revertStage(s, 2)} title="Xato belgilangan bo‘lsa — bosib bekor qiling"><Pill tone="amber">Hujjat topshirgan ✕</Pill></button> : <Pill tone={s.hujjat_topshirgan ? "amber" : "gray"}>{s.hujjat_topshirgan ? "Hujjat topshirgan" : "Hujjat topshirmagan"}</Pill>}{s.bazaga_kiritilgan && !s.saytga_kirgan && permissions.bazaga_belgilash ? <button type="button" onClick={() => revertStage(s, 3)} title="Xato belgilangan bo‘lsa — bosib bekor qiling"><Pill tone="violet">Bazaga kiritilgan ✕</Pill></button> : <Pill tone={s.bazaga_kiritilgan ? "violet" : "gray"}>{s.bazaga_kiritilgan ? "Bazaga kiritilgan" : "Bazaga kiritilmagan"}</Pill>}{permissions.bazaga_belgilash && s.bazaga_kiritilgan ? (s.saytga_kirgan
      ? <button type="button" onClick={() => revertStage(s, 4)} title="Qo‘lda qo‘yilgan bo‘lsa — bosib bekor qiling (haqiqiy kirgan bo‘lsa qaytarilmaydi)"><Pill tone="green">Saytga kirgan ✕</Pill></button>
      : <button type="button" onClick={() => markStage(s.id, 4)} title="Talaba saytga kirdi deb belgilash"><Pill tone="gray">Saytga kirmagan · belgilash</Pill></button>)
      : <Pill tone={s.saytga_kirgan ? "green" : "gray"}>{s.saytga_kirgan ? "Saytga kirgan" : "Saytga kirmagan"}</Pill>}{!s.hujjat_topshirgan && permissions.hujjat_belgilash && <button onClick={() => markStage(s.id, 2)}><Pill tone="amber">✓ Hujjatni belgilash</Pill></button>}{s.hujjat_topshirgan && !s.bazaga_kiritilgan && permissions.bazaga_belgilash && <button onClick={() => markStage(s.id, 3)}><Pill tone="violet">✓ Bazaga kiritish</Pill></button>}{!s.saytga_kirgan && permissions.parol_korish && <button onClick={() => revealPassword(s.id)}><Pill tone="blue"><Eye size={12} className="mr-1" /> Parolni ko‘rish</Pill></button>}{s.bazaga_kiritilgan && !s.saytga_kirgan && permissions.parol_korish && <button onClick={() => invite(s.id)}><Pill tone="amber">SMS yuborish</Pill></button>}{s.bazaga_kiritilgan && !s.saytga_kirgan && permissions.saytga_kiritish && <button onClick={() => markStage(s.id, 4)}><Pill tone="green">✓ Saytga kirdi</Pill></button>}{s.saytga_kirgan && <Pill tone="gray">Parol yopildi</Pill>}</> : <>{s.bazaga_kiritilgan ? (!s.saytga_kirgan && canMarkHemis ? <button type="button" onClick={() => revertStage(s, 3)} title="Xato belgilangan bo‘lsa — bosib bekor qiling"><Pill tone="violet">Bazaga kiritilgan ✕</Pill></button> : <Pill tone="violet">Bazaga kiritilgan</Pill>) : canMarkHemis && (s.bazaga_belgilash_mumkin || s.hujjat_topshirgan) ? <button onClick={() => markStage(s.id, 3)}><Pill tone="violet">✓ Bazaga kiritish</Pill></button> : <Pill tone="gray">Admin hujjatni tasdiqlashi kutilmoqda</Pill>}{s.saytga_kirgan ? <Pill tone="green">Saytga kirgan</Pill> : s.bazaga_kiritilgan && permissions.saytga_kiritish ? <button onClick={() => markStage(s.id, 4)}><Pill tone="green">✓ Saytga kirdi</Pill></button> : <Pill tone="gray">Saytga kirmagan</Pill>}</>}</div></td></tr>)}</tbody></table></div>}</Card>
...
function DocumentImportPanel({ apiBase, token, universityId, onDone, canHemis = false }) {
  ...
  return <Card className="p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><Pill tone="amber">OMMAVIY AMAL</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>Excel orqali ommaviy belgilash / olib tashlash</h3><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>Shablonni yuklang → A ustuniga abituriyentlarning <b>AbiturID</b> larini qo‘ying → amalni tanlang → faylni yuklang. Avval tekshiriladi, keyin siz tasdiqlaysiz.</p></div>
      <a href={templateUrl} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-black" style={{ borderColor: COLORS.line, color: COLORS.blue }}><Download size={16} /> Shablonni yuklash</a>
    </div>
    <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{DOC_IMPORT_ACTIONS.filter(([key]) => canHemis || !key.startsWith("baza")).map(([key, label, tone, hint]) => <button type="button" key={key} onClick={() => { setAction(key); setPreview(null); setDone(null); }} className="rounded-2xl border p-3 text-left" style={{ borderColor: action === key ? COLORS.blue : COLORS.line, background: action === key ? COLORS.sky : "#fff" }}><Pill tone={tone}>{label}</Pill><div className="mt-1 text-[11px]" style={{ color: COLORS.muted }}>{hint}</div></button>)}</div>
    <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
      <label className="flex cursor-pointer items-center gap-3 rounded-2xl border-2 border-dashed p-4" style={{ borderColor: COLORS.line }}>
        ...
        <span className="min-w-0 flex-1 text-sm" style={{ color: file ? COLORS.ink : COLORS.muted }}>{file ? file.name : "Excel faylni tanlang (.xlsx / .xls) — A ustunida AbiturID"}</span>
      ...
      <Button onClick={() => send(false)} disabled={!file || busy}>{busy && !preview ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />} Tekshirish</Button>
    </div>
    <ErrorBox text={error} />
    {preview && <div className="mt-4 space-y-3">
      <div className="grid gap-3 sm:grid-cols-5">{[["Fayldagi ID", preview.jami_id, "blue"], ["Bajariladi", preview.belgilanadi, "green"], ["Allaqachon shunday", preview.allaqachon, "amber"], ["Bloklandi", preview.bloklandi || 0, "gray"], ["Topilmadi", preview.topilmadi, "red"]].map(([label, n, tone]) => <div key={label} className="rounded-2xl border p-3" style={{ borderColor: COLORS.line }}><Pill tone={tone}>{label}</Pill><div className="mt-1 text-2xl font-black" style={{ color: COLORS.ink }}>{n}</div></div>)}</div>
      {preview.belgilanadi > 0 && <div className="rounded-2xl border p-3" style={{ borderColor: COLORS.line }}><div className="mb-2 text-xs font-black" style={{ color: COLORS.green }}>{preview.amal_nomi} — bajariladi ({preview.belgilanadi}):</div><div className="max-h-48 overflow-auto text-xs" style={{ color: COLORS.ink }}>{preview.belgilanadi_royxat.map((r, i) => <div key={r.abitur_id + i} className="flex justify-between gap-2 border-t py-1" style={{ borderColor: COLORS.line }}><span>{r.abitur_id} · <b>{r.fish}</b></span><span style={{ color: COLORS.muted }}>{r.yonalish}</span></div>)}{preview.belgilanadi > preview.belgilanadi_royxat.length && <div className="pt-1" style={{ color: COLORS.muted }}>... va yana {preview.belgilanadi - preview.belgilanadi_royxat.length} ta</div>}</div></div>}
      {preview.bloklandi > 0 && <div className="rounded-2xl border p-3" style={{ borderColor: COLORS.line, background: COLORS.cream }}><div className="mb-1 text-xs font-black" style={{ color: COLORS.muted }}>Bloklandi ({preview.bloklandi}) — tartib qoidasi tufayli o‘tkazib yuboriladi:</div><div className="max-h-32 overflow-auto text-xs" style={{ color: COLORS.ink }}>{preview.bloklandi_royxat.map((r, i) => <div key={r.abitur_id + i}>{r.abitur_id} · {r.fish} — <i>{r.sabab}</i></div>)}</div></div>}
      {preview.topilmadi > 0 && <div className="rounded-2xl border p-3" style={{ borderColor: "#F1C2C2", background: "#FFF7F7" }}><div className="mb-1 text-xs font-black" style={{ color: COLORS.red }}>Bazada topilmadi ({preview.topilmadi}) — ID xato yoki bu yo‘nalishlarga import qilinmagan:</div><div className="max-h-32 overflow-auto text-xs" style={{ color: COLORS.ink }}>{preview.topilmadi_royxat.join(", ")}{preview.topilmadi > preview.topilmadi_royxat.length ? " ..." : ""}</div></div>}
      <div className="flex flex-wrap items-center gap-2">
        <Button onClick={() => send(true)} disabled={busy || !preview.belgilanadi}>{busy ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />} {preview.belgilanadi} ta talaba: {preview.amal_nomi}</Button>
        <span className="text-xs" style={{ color: COLORS.muted }}>“Allaqachon shunday” va “Bloklandi” o‘zgarmaydi. Xato qilsangiz — teskari amal bilan yoki ro‘yxatdagi ✕ bilan qaytarasiz.</span>
      </div>
    </div>}
    {done && <div className="mt-4 rounded-2xl p-4 text-sm" style={{ background: "#EAF7EF", color: "#28734B" }}>✅ {done.amal_nomi}: {done.belgilandi} ta talaba{done.allaqachon ? ` · ${done.allaqachon} tasi allaqachon shunday edi` : ""}{done.bloklandi ? ` · ${done.bloklandi} ta bloklandi` : ""}{done.topilmadi ? ` · ${done.topilmadi} ta ID topilmadi` : ""}.</div>}
  </Card>;
}
...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false, initialFacultyId = null }) {
  ...
  if (!bootstrap) {
    if ((workspaceId || legacyUniversityId || manualUniversityId) && error) return <main className="mx-auto max-w-2xl p-6"><button onClick={onBack} className="mb-4 inline-flex items-center gap-2 text-sm font-black" style={{ color: COLORS.blue }}><ArrowLeft size={17} /> Bosh sahifa</button><ErrorBox text={error} /></main>;
  ...
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
        ...
        <header className="mb-4 overflow-hidden rounded-3xl border" style={{ borderColor: COLORS.line, background: "linear-gradient(135deg,#153E5B,#14797A)", color: "#fff" }}>
          <div className="flex flex-col gap-5 p-5 md:flex-row md:items-center md:justify-between md:p-7">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="text-xs font-black tracking-[.18em]" style={{ color: "rgba(255,255,255,.78)" }}>INSTITUT ISH MAYDONI</div>
                <h1 className="mt-1 text-2xl font-black md:text-3xl" style={{ color: "#fff" }}>{bootstrap.universitet.nomi}</h1>
                {activeFaculty && <div className="mt-1 text-sm font-bold" style={{ color: "rgba(255,255,255,.82)" }}>Tanlangan fakultet: {activeFaculty.nomi}</div>}
      ...
      {tab === "dashboard" && <div className="space-y-4">{activeFaculty ? <>
        <Card className="p-5 md:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><Pill tone="blue">FAKULTET</Pill><h2 className="mt-2 text-xl font-black" style={{ color: COLORS.ink }}>{activeFaculty.nomi}</h2><p className="mt-1 text-sm" style={{ color: COLORS.muted }}>Fakultet haqida qisqa ma’lumot. Talaba ro‘yxati faqat yo‘nalish tanlangandan keyin ochiladi.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => navigateTab("structure")}><Building2 size={16} /> Kafedra va yo‘nalishlar</Button>{permissions.qabul_korish && <Button kind="secondary" onClick={() => { setAdmissionStartMode("list"); navigateTab("admission"); }}><ClipboardCheck size={16} /> Hujjat topshirish natijalari</Button>}{canViewReportsHere && <Button kind="secondary" onClick={() => { setAdmissionStartMode("reports"); navigateTab("admission"); }}><FileSpreadsheet size={16} /> Qabul hisoboti</Button>}</div></div><div className="mt-5 grid gap-3 sm:grid-cols-3 xl:grid-cols-6">{[[activeFacultyStats.kafedra, "Kafedra", () => navigateTab("structure")], [activeFacultyStats.mudir, "Kafedra mudiri", permissions.xodim_korish ? () => navigateTab("staff") : null], [activeFacultyStats.oqituvchi, "O‘qituvchi", permissions.xodim_korish ? () => navigateTab("staff") : null], [activeFacultyStats.tyutor, "Tyutor", permissions.tyutor_korish ? () => navigateTab("tutors") : null], [activeFacultyStats.yonalish, "Yo‘nalish", () => navigateTab("structure")], [activeFacultyStats.talaba, "Talaba", permissions.qabul_korish ? () => { setAdmissionStartMode("list"); navigateTab("admission"); } : null]].map(([n, label, go]) => <button key={label} type="button" disabled={!go} onClick={go || undefined} className="rounded-2xl p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-default disabled:hover:translate-y-0 disabled:hover:shadow-none" style={{ background: COLORS.sky }}><div className="text-2xl font-black" style={{ color: COLORS.blue }}>{n}</div><div className="text-xs font-bold" style={{ color: COLORS.muted }}>{label}{go ? " ›" : ""}</div></button>)}</div></Card>
