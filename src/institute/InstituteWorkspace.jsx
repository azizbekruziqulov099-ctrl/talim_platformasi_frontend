...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, onOpenStaff, startMode, onStartModeConsumed }) {
  ...
  if (selectedFaculty) return <div className="space-y-4">
    ...
    <Card className="p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><Pill tone="green">YO‘NALISHLAR</Pill><h3 className="mt-2 font-black" style={{ color: COLORS.ink }}>Import qilingan talabalar shu yerda saqlanadi</h3></div><Pill tone="blue">{selectedFaculty.yonalish_soni || 0} ta</Pill></div><div className="mt-4 grid gap-3 md:grid-cols-2">{selectedFaculty.kafedralar.flatMap(d => d.yonalishlar.map(y => <div key={y.id} className="rounded-2xl border p-4" style={{ borderColor: COLORS.line }}><div className="font-black" style={{ color: COLORS.blue }}>{y.nomi}</div><div className="mt-1 text-xs" style={{ color: COLORS.muted }}>{y.daraja} · {d.nomi}</div></div>))}{!selectedFaculty.yonalish_soni && <Empty>Hali yo‘nalish yo‘q. Pastdan Excel import qiling.</Empty>}</div></Card>
