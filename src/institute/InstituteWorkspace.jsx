...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, startMode, onStartModeConsumed }) {
  ...
  if (selectedFaculty) return <div className="space-y-4">
    ...
    <Card className="p-5" style={{ background: "linear-gradient(135deg,#EAF5F8,#FBF7EE)" }}><Pill tone="blue">FAKULTET</Pill><h2 className="mt-2 text-2xl font-black" style={{ color: COLORS.ink }}>{selectedFaculty.nomi}</h2><p className="mt-1 text-sm" style={{ color: COLORS.muted }}>{selectedFaculty.kafedra_soni} kafedra · {selectedFaculty.yonalish_soni} yo‘nalish</p></Card>
...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false }) {
  ...
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
      ...
      {tab === "structure" && <StructurePanel api={api} apiBase={apiBase} token={token} universityId={id} canManage={permissions.tuzilma_boshqarish} permissions={permissions} onChanged={() => load()} onCredentials={setCredentials} onNextAdmission={() => { setAdmissionStartMode("import"); setTab("admission"); }} startMode={structureStartMode} onStartModeConsumed={() => setStructureStartMode(null)} />}
