...
function StructurePanel({ api, apiBase, token, universityId, canManage, permissions, onChanged, onCredentials, onNextAdmission, onOpenStaff, onBackToWorkspace, onInitialFacultyBack, onProgramDepthChange, onFacultyContextChange, startMode, onStartModeConsumed, focusFacultyId = null, facultyContextLocked = false }) {
  ...
  if (selectedFaculty && selectedProgram) return <div className="space-y-4">
    ...
    <div><Pill tone="green">TA’LIM YO‘NALISHI</Pill><h2 className="mt-2 text-xl font-black" style={{ color: COLORS.ink }}>{selectedProgram.nomi}</h2><p className="mt-1 text-xs" style={{ color: COLORS.muted }}>{selectedProgram.daraja || "Daraja belgilanmagan"} · {selectedProgram.kafedra_nomi}</p></div>
    <AdmissionsPanel compactProgram api={api} apiBase={apiBase} token={token} universityId={universityId} structure={data} permissions={permissions} onCredentials={onCredentials} startMode="list" lockedFacultyId={selectedFaculty.id} lockedProgramId={selectedProgram.id} lockedProgramIds={selectedProgram.alias_ids || null} />
  </div>;
  if (selectedFaculty) return <div className="space-y-4">
    ...
    <AdmissionsPanel api={api} apiBase={apiBase} token={token} universityId={universityId} structure={data} permissions={permissions} onCredentials={onCredentials} startMode="list" lockedFacultyId={selectedFaculty.id} showDailyReport />
  </div>;
  return <div className="space-y-4">
...
function StaffPanel({ api, token, universityId, structure, canManage, canManageAdmins, isSuperAdmin, onCredentials, facultyId = null }) {
  const scopedFacultyId = facultyId ? String(facultyId) : "";
  const [staff, setStaff] = useState([]); const [form, setForm] = useState({ fish: "", telefon: "", rol: "dekan", fakultet_id: scopedFacultyId, kafedra_id: "", yonalish_id: "" });
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!editingId) onCredentials([data]); setEditingId(null); setForm({ fish: "", telefon: "", rol: "dekan", fakultet_id: scopedFacultyId, kafedra_id: "", yonalish_id: "" }); load();
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  };
...
function AdmissionsPanel({ api, apiBase, token, universityId, structure, permissions, onCredentials, startMode, onStartModeConsumed, lockedFacultyId = null, lockedProgramId = null, lockedProgramIds = null, compactProgram = false, showDailyReport = true }) {
  ...
  const [mode, setMode] = useState("list"); const [tab, setTab] = useState(compactProgram ? 0 : (fullStatuses ? 1 : 0)); const [items, setItems] = useState([]); const [counts, setCounts] = useState({});
  ...
  useEffect(() => { if (!fullStatuses || compactProgram) setTab(0); }, [compactProgram, fullStatuses]);
  ...
  }, [api, universityId, token, tab, page, filters, selectedFacultyId, lockedProgramId, lockedProgramIds, permissions.qabul_korish, programs]);
  useEffect(() => { load(); }, [load]);
...
export default function InstituteWorkspace({ token, apiBase, initialWorkspace, onBack, canCreateInstitution = false, initialFacultyId = null }) {
  ...
  const heroStats = activeFacultyStats
    ? [[activeFacultyStats.kafedra, "Kafedra"], [activeFacultyStats.yonalish, "Yo‘nalish"], [activeFacultyStats.talaba, "Talaba"]]
    : [[bootstrap.sonlar.fakultet, "Fakultet"], [canonicalProgramCount, "Yo‘nalish"], [bootstrap.sonlar.talaba, "Qabul"]];
  return <main className="min-h-screen" style={{ background: "linear-gradient(180deg,#F4FAFB 0,#FAF8F3 48%,#F7FAFB 100%)" }}>
    <div className="mx-auto max-w-[1500px] p-3 md:p-6">
          ...
          <div className="flex flex-col gap-5 p-5 md:flex-row md:items-center md:justify-between md:p-7">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2" style={{ color: "#fff" }}>{permissions.super_admin && <Pill tone="violet">SUPER ADMIN</Pill>}<Pill>{bootstrap.rollar.map(r => r.rol).join(" · ")}</Pill><span className="text-xs" style={{ color: "rgba(255,255,255,.72)" }}>{bootstrap.universitet.viloyat} {bootstrap.universitet.tuman}</span></div>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2">{heroStats.map(([n, l]) => <div key={l} className="rounded-2xl px-4 py-3 text-center" style={{ background: "rgba(255,255,255,.12)", color: "#fff" }}><div className="text-xl font-black">{n}</div><div className="text-[10px] font-bold" style={{ color: "rgba(255,255,255,.72)" }}>{l}</div></div>)}</div>
          </div>
        </header>
      ...
      {tab === "staff" && <StaffPanel api={api} token={token} universityId={id} structure={structure} canManage={permissions.xodim_boshqarish} canManageAdmins={permissions.admin_boshqarish} isSuperAdmin={permissions.super_admin} onCredentials={setCredentials} facultyId={activeFacultyId} />}
      {tab === "admission" && <AdmissionsPanel api={api} apiBase={apiBase} token={token} universityId={id} structure={structure} permissions={permissions} onCredentials={setCredentials} startMode={admissionStartMode} onStartModeConsumed={() => setAdmissionStartMode(null)} lockedFacultyId={activeFacultyId} showDailyReport={!activeFacultyId} />}
