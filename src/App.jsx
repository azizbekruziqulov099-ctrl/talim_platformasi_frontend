...
function haqiqiyRasmKodimi(qiymat) {
  ...
  if (/^https?:\/\//i.test(q)) return true; // image_url ustunidagi to'g'ridan-to'g'ri internet havolasi
...
function SavolRasmi({ rasmId }) {
  ...
  const q = String(rasmId || "").trim();
  const rasmSrc = /^https?:\/\//i.test(q)
    ? q
    : q.startsWith("/api/")
      ? `${API_BASE}${q}`
      : `${API_BASE}/api/rasm/${encodeURIComponent(q)}`;

  if (holat === "xato") {
    return (
      <div className="w-full rounded-xl mb-4 px-4 py-3 text-center text-xs"
        style={{ backgroundColor: "#FCEBEB", color: "#A32D2D", border: "1px solid #F3D3D3" }}>
        Rasm yuklanmadi. Administrator rasm diagnostikasini tekshirsin.
      </div>
    );
  }
  return (
      ...
      <div className="rounded-2xl p-5 bg-white border" style={{ borderColor: "#E5E1D8" }}>
        ...
        {natija && (
          <div className="mt-3 text-sm" style={{ color: "#2B2B2B" }}>
            ...
            {natija.errors_detail?.length > 0 && (
              <div className="mt-1.5 rounded-lg px-2.5 py-2 text-xs"
                style={{ backgroundColor: "#FCEBEB", color: "#A32D2D" }}>
                <p className="font-semibold mb-1">Import xatolari:</p>
                {natija.errors_detail.map((x, i) => (
                  <p key={i} className="font-mono" style={{ wordBreak: "break-all" }}>{x}</p>
                ))}
              </div>
            )}
            ...
            {natija.rasm_diagnostika && (
              <div className="mt-1.5 rounded-lg px-2.5 py-2 text-xs" style={{ backgroundColor: "#EAF1F7", color: "#1B4B7A" }}>
                ...
                <p>XLSX media papkasidagi rasm: <b>{natija.rasm_diagnostika.xlsx_media_fayllari_soni ?? 0}</b></p>
        ...
        <div className="space-y-2">
          {qatorlar.map((q, i) => (
            <div key={q.qator_id} className="rounded-xl p-3 bg-white border flex items-center gap-2.5" style={{ borderColor: "#E5E1D8" }}>
              <span className="text-xs font-bold w-6 text-center shrink-0" style={{ color: "#8A8578" }}>{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: "#2B2B2B" }}>{q.mavzu_name || q.kichik_name}</p>
                <p className="text-xs truncate" style={{ color: "#8A8578" }}>{[q.bob_name, q.bolim_name].filter(Boolean).join(" · ")}</p>
              </div>
  ...
  const rejaYarat = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/oqituvchi/reja_yarat`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, nomi: yangiNomi.trim(), sinf: sinfQiymati, fan: fanQiymati.trim() }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || "Xato");
          ...
          {rejalar.map((r) => (
            <button key={r.id} onClick={() => setTanlanganRejaId(r.id)}
              ...
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: "#8A8578" }}>{r.sinf}-{r.fan} · {r.mavzu_soni} ta mavzu</p>
              </div>
              <ChevronRight size={16} style={{ color: "#8A8578" }} />
            </button>
          ))}
...
function OqituvchiTab({ token, foydalanuvchi, boshlanishKorinishi }) {
  ...
  const yangiRejaYaratishBoshla = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/oqituvchi/reja_yarat`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, nomi: yangiRejaNomi.trim(), sinf: sinfQiymati, fan: yangiFan }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || "Xato");
      setYangiRejaNomi("");
