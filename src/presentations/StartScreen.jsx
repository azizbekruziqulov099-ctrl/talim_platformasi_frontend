...
export function AudiencePicker({ value, onChange }) {
  ...
  return <div className="ps60-audience">
    <div className="ps60-audience-groups" role="group" aria-label="Auditoriya turi">
      {[['maktab', '🏫 Maktab'], ['bakalavr', '📘 Bakalavr'], ['magistr', '🎓 Magistr'], ['boshqa', '✏️ Boshqa']].map(([id, name]) => <button type="button" key={id} className={group === id ? 'is-active' : ''} aria-pressed={group === id} onClick={() => { setGroup(id); if (id !== 'boshqa') onChange(''); }}>{name}</button>)}
    </div>
    {group === 'boshqa' ? <input value={value || ''} onChange={event => onChange(event.target.value)} maxLength={120} placeholder="Masalan: ota-onalar yig‘ilishi, kollej 2-bosqich" />
      : <div className="ps60-audience-numbers">{numbers.map(number => <button type="button" key={number} className={parsed.number === number && parsed.group === group ? 'is-active' : ''} aria-pressed={parsed.number === number && parsed.group === group} onClick={() => onChange(label(number))}>{group === 'maktab' ? number : `${number}-kurs`}</button>)}</div>}
  </div>;
}
...
function BackgroundTile({ theme, mode, accent, selected, onSelect }) {
  ...
  return <button type="button" className={`ps60-bg-tile${selected ? ' is-selected' : ''}`} aria-pressed={selected} onClick={onSelect} title={theme.tagline}>
    ...
    <span className="ps60-bg-name">{theme.emoji} {theme.label}</span>
  </button>;
}
...
export function BackgroundGallery({ value, mode, accent = 'blue', onSelect, onMode, onUpload, ownImage, busy, compact = false, onClear }) {
  ...
  return <div className={`ps60-bg-gallery${compact ? ' is-compact' : ''}`}>
    <div className="ps60-bg-head">
      <div className="ps49-segments ps60-bg-mode" role="group" aria-label="Fon yorug‘ligi">{[['light', 'Och fon'], ['dark', 'To‘q fon']].map(([id, name]) => <button type="button" key={id} className={mode === id ? 'is-active' : ''} aria-pressed={mode === id} onClick={() => onMode(id)}>{name}</button>)}</div>
      {onUpload && <><button type="button" className="ps49-button ps49-button-small" disabled={!!busy} onClick={() => document.getElementById(fileId)?.click()}><Upload size={14} />{ownImage ? 'Boshqa rasmim' : 'O‘z rasmim'}</button><input id={fileId} className="ps49-file-input" tabIndex={-1} type="file" accept="image/png,image/jpeg" disabled={!!busy} onChange={event => { const file = event.target.files?.[0]; event.target.value = ''; if (file) onUpload(file); }} /></>}
    </div>
    ...
    {onClear && <button type="button" className="ps49-text-button" onClick={onClear}>Fonsiz (oddiy rang)</button>}
  </div>;
}
...
export default function StartScreen({ draft, updateDraft, count, setCount, mode, setMode, aiAvailable, aiInstructions, setAiInstructions, bgTheme, setBgTheme, bgMode, setBgMode, onStart, busy, aiRunning, advanced }) {
  ...
  return <div className="ps60-start">
    <section className="ps49-card ps60-start-card">
      <h2>Taqdimotni qanday tayyorlaymiz?</h2>
      <p className="ps49-intro">Bittasini tanlang. Keyin istalgan vaqtda boshqasiga o‘tsangiz bo‘ladi.</p>
      <div className="ps60-modes" role="radiogroup" aria-label="Tayyorlash usuli">
        {START_MODES.map(item => { const Icon = item.icon; const disabled = item.id === 'ai' && !aiAvailable; return <button type="button" key={item.id} role="radio" aria-checked={mode === item.id} className={`ps60-mode${mode === item.id ? ' is-active' : ''}`} onClick={() => setMode(item.id)} disabled={disabled}>
          ...
          <small>{disabled ? 'AI xizmati hozir yoqilmagan — boshqa usulni tanlang.' : item.text}</small>
          {mode === item.id && <i className="ps60-mode-check"><Check size={14} /></i>}
        </button>; })}
      </div>

      <div className="ps60-fields">
        <label className="ps60-field"><span>Taqdimot mavzusi <b>*</b></span><input value={draft.title} onChange={event => updateDraft(previous => ({ ...previous, title: event.target.value }), 'topic')} maxLength={160} placeholder="Masalan: Kasrlarni qo‘shish va ayirish" autoFocus /></label>
        <div className="ps60-two">
          <label className="ps60-field"><span>Fan</span><input value={draft.subject} onChange={event => updateDraft(previous => ({ ...previous, subject: event.target.value }), 'subject')} maxLength={100} placeholder="Masalan: Matematika" /></label>
          <label className="ps60-field"><span>Dars turi</span><select value={draft.lesson_type} onChange={event => updateDraft(previous => ({ ...previous, lesson_type: event.target.value }))}>{Object.entries(LESSONS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        </div>
        <div className="ps60-field"><span>Kimga? (sinf yoki kurs)</span><AudiencePicker value={draft.audience || ''} onChange={audience => updateDraft(previous => ({ ...previous, audience }), 'audience')} /></div>
        <div className="ps60-field"><span>Nechta slayd?</span>
          <div className="ps60-counts">{COUNTS.map(value => <button type="button" key={value} className={count === value ? 'is-active' : ''} aria-pressed={count === value} onClick={() => setCount(value)}>{value}</button>)}<label className="ps60-count-input">yoki<input type="number" min={3} max={40} value={count} onChange={event => { const value = parseInt(event.target.value, 10); if (Number.isFinite(value)) setCount(Math.max(3, Math.min(40, value))); }} aria-label="Slaydlar soni" />ta</label></div>
          <p className="ps49-help">Kirish, asosiy qism, mustahkamlash va xulosa avtomatik taqsimlanadi. Keyin slayd qo‘shish yoki olib tashlash mumkin.</p>
        </div>
      ...
      <div className="ps60-bg-pick">
        <div className="ps60-bg-pick-head">
          ...
          <div>
            <strong>Orqa fon: {bgTheme === 'none' ? 'fonsiz' : `${themeMeta?.label || 'Ta’lim'}${bgTheme === 'auto' ? ' · fan bo‘yicha avtomatik' : ''}`}</strong>
            <p className="ps49-help">Fan yoki sinfni yozsangiz, fon o‘zi mos tanlanadi. Tahrirda istalgan vaqt o‘zgartirasiz.</p>
          </div>
          <button type="button" className="ps49-button ps49-button-small" onClick={() => setBgOpen(open => !open)} aria-expanded={bgOpen}>{bgOpen ? 'Yopish' : 'Fonni tanlash'}<ChevronDown size={14} style={{ transform: bgOpen ? 'rotate(180deg)' : 'none' }} /></button>
        </div>
      ...
      {!draft.title.trim() && <p className="ps49-help">Boshlash uchun mavzuni yozing.</p>}
    </section>

    <aside className="ps60-side">
      <section className="ps49-card ps60-howto">
        <h3>{active.title} — qanday ishlaydi?</h3>
        <ol>{active.steps.map((step, index) => <li key={index}><span>{index + 1}</span>{step}</li>)}</ol>
        {mode === 'word' && <p className="ps49-help">Word shablonda har slayd uchun nima yozish kerakligi, kimga (sinf/kurs), dars turi va so‘z chegaralari yozilgan bo‘ladi. AI to‘ldirgan faylni qaytarib yuklasangiz, matn o‘z joyiga tushadi.</p>}
