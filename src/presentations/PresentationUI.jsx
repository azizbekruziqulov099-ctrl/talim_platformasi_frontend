...
export function OverlayPortal({ children, onClose, onKeyDown, className = '', panelRef, label, labelledBy, backdrop = false }) {
  ...
  return createPortal(<div ref={node => { ownRef.current = node; if (panelRef) panelRef.current = node; }} className={`ps49-studio ps50-portal ${className}`} role="dialog" aria-modal="true" aria-label={label} aria-labelledby={labelledBy} tabIndex={-1} onMouseDown={event => { if (backdrop && event.target === event.currentTarget) onClose?.(); }}>{children}</div>, document.body);
}

export function StudioDialog({ title, eyebrow, children, onClose, actions, className = '' }) {
  const titleId = useId();
  return <OverlayPortal className="ps49-modal-backdrop" onClose={onClose} labelledBy={titleId} backdrop><section className={`ps49-modal ${className}`}><header className="ps49-modal-heading"><div>{eyebrow && <p className="ps49-eyebrow">{eyebrow}</p>}<h2 id={titleId}>{title}</h2></div><button type="button" className="ps49-button" data-overlay-close onClick={onClose}><X size={18} />Yopish</button></header><div className="ps50-modal-body">{children}</div><footer className="ps49-modal-actions"><button type="button" className="ps49-button" onClick={onClose}>Yopish</button>{actions}</footer></section></OverlayPortal>;
}
...
function TemplateCard({ template, selected, onSelect }) {
  ...
  return <article className={`ps50-template-card${selected ? ' is-selected' : ''}`} onMouseEnter={() => play()} onMouseLeave={stop} onFocus={event => { if (!event.currentTarget.contains(event.relatedTarget)) play(); }} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) stop(); }}>
    <button type="button" className="ps50-template-select" aria-label={`${template.label} maketini tanlash`} aria-pressed={selected} onClick={() => onSelect(template)}><div className="ps50-template-preview"><SlidePreview document={doc} index={frame} thumbnail transition={playing} /></div><span className="ps50-template-name">{template.label}{selected && <Check size={17} />}</span><span className="ps50-template-description">{template.description}</span></button>
    <div className="ps50-template-bottom"><span aria-hidden="true">{[0, 1, 2].map(index => <i key={index} className={index === frame ? 'is-active' : ''} />)}</span><button type="button" className="ps49-text-button" onClick={() => play(true)} aria-label={`${template.label}: 3 ko‘rinish namoyishi`}><MonitorPlay size={15} />{playing ? 'Ko‘rsatilmoqda' : '3 ko‘rinish'}</button></div>
  </article>;
}
...
export function LayoutPicker({ value, onChange }) {
  return <div className="ps49-field"><span className="ps49-label">Slayd joylashuvi va maydonlar soni</span><div className="ps50-layout-picker" role="group" aria-label="Slayd joylashuvi">{LAYOUTS.map(layout => <button type="button" key={layout.id} aria-pressed={value === layout.id} className={value === layout.id ? 'is-active' : ''} onClick={() => onChange(layout.id)}><span className={`ps50-layout-icon is-${layout.id}`} aria-hidden="true">{Array.from({ length: layout.textCount }, (_, i) => <i key={`text${i}`} />)}{Array.from({ length: layout.imageCount }, (_, i) => <b key={`image${i}`} />)}</span><strong>{layout.label}</strong><small>{layout.textCount} matn · {layout.imageCount} rasm{layout.id === 'cover' ? ' (ixtiyoriy)' : ''}</small></button>)}</div><p className="ps49-help">Joylashuvni almashtirish yashirin maydonlardagi matn va rasmlarni saqlaydi.</p></div>;
}
