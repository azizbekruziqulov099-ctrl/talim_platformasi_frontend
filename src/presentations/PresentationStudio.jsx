...
function DesignControls({ draft, selected, scope, setScope, changeDesign, resetSlide, uploadBackground, busy }) {
  ...
  return <section className="ps49-card ps49-design-controls" aria-label="Dizayn sozlamalari">
    ...
    <SelectField label="Orqa fon" value={design.background} onChange={background => changeDesign({ background, overlay: background === 'paper' || background === 'solid' ? 0 : design.overlay })}><option value="paper">Och fon</option><option value="midnight">To‘q fon</option><option value="aurora">Yumshoq ranglar</option><option value="solid">Rang tanlash</option><option value="image">O‘z rasmim</option></SelectField>
    {design.background === 'solid' && <Field label="Fon rangi" value={design.color} type="color" onChange={color => changeDesign({ color })} />}
    {design.background === 'image' && <div className="ps49-image-upload"><button type="button" className="ps49-button" disabled={!!busy} onClick={() => document.getElementById(fileId)?.click()}><Upload size={15} />{design.image ? 'Rasmni almashtirish' : 'Fon rasmini tanlash'}</button><input id={fileId} className="ps49-file-input" tabIndex={-1} type="file" accept="image/png,image/jpeg" disabled={!!busy} onChange={event => { const file = event.target.files?.[0]; event.target.value = ''; if (file) uploadBackground(file); }} /><p className="ps49-help">PNG yoki JPG · 2 MB gacha moslashtiriladi.</p>{design.image && <button type="button" className="ps49-text-button" onClick={() => changeDesign({ image: null, background: 'paper' })}>Fon rasmini olib tashlash</button>}</div>}
...
export default function PresentationStudio({ apiBase = '', token, user, active = true, onClose }) {
  ...
  const goStep = value => {
    if (step === 0 && value > 0) {
      ...
      if (needsOutline && fillMode === 'ai' && aiAvailable) generateAI(undefined, draftRef.current);
    }
  ...
  return <div className="ps49-studio" ref={rootRef}>
    ...
    {!capabilities ? <div className="ps49-empty-state">{capError ? <><AlertCircle size={32} /><h2>Ulanishni tekshiring</h2><p role="alert">{capError}</p><button type="button" className="ps49-button" onClick={() => setCapAttempt(value => value + 1)}>Qayta urinish</button></> : <><LoaderCircle className="ps49-spin" size={30} /><p role="status">Taqdimot imkoniyatlari tekshirilmoqda…</p></>}</div> : <>
      ...
      {!capabilities.allowed ? <div className="ps49-empty-state"><Presentation size={40} /><h2>Taqdimot yaratish uchun ruxsat kerak</h2><p>{capabilities.reason || 'Hisobingiz uchun bu bo‘lim hali yoqilmagan.'}</p><p>Ruxsat masalasida muassasa administratoriga murojaat qiling.</p></div> : <>
        ...
        {step === 0 && <div className="ps49-setup"><section className="ps49-card ps49-setup-form"><p className="ps49-eyebrow">1-QADAM</p><h2>Dars mavzusidan boshlang</h2><p className="ps49-intro">Mavzu va auditoriyani kiriting. Mazmunni o‘zingiz yozing yoki AI yordamida tayyorlang.</p>
          <Field label="Taqdimot mavzusi" value={draft.title} onChange={title => updateDraft(previous => ({ ...previous, title }), 'topic')} maxLength={160} placeholder="Masalan: Bir tekis to‘g‘ri chiziqli harakat" />
          <div className="ps49-two-fields"><Field label="Fan yoki kurs" value={draft.subject} onChange={subject => updateDraft(previous => ({ ...previous, subject }), 'subject')} maxLength={100} placeholder="Masalan: Fizika" /><Field label="Sinf, kurs yoki auditoriya" value={draft.audience || ''} onChange={audience => updateDraft(previous => ({ ...previous, audience }), 'audience')} maxLength={120} placeholder="Masalan: 9-sinf yoki 2-kurs" /></div>
          <SelectField label="Dars turi" value={draft.lesson_type} onChange={lesson_type => updateDraft(previous => ({ ...previous, lesson_type }))}>{Object.entries(LESSONS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField>
          <div className="ps49-field"><span className="ps49-label">Nechta slayd kerak?</span><div className="ps49-counts" role="group" aria-label="Slaydlar soni">{[...new Set([...COUNTS, count])].sort((a, b) => a - b).map(value => <button type="button" key={value} className={count === value ? 'is-active' : ''} onClick={() => setCount(value)} aria-pressed={count === value}>{value}</button>)}</div><p className="ps49-help">Kirish, asosiy qism, mustahkamlash va xulosa. Keyin slayd qo‘shish yoki olib tashlash mumkin.</p></div>
          <Segments label="Mazmunni qanday tayyorlaysiz?" value={fillMode} onChange={setFillMode} options={[{ value: 'manual', label: 'O‘zim to‘ldiraman' }, { value: 'ai', label: 'AI yordamida' }]} />
          {fillMode === 'ai' && <div className="ps51-ai-brief">{aiAvailable ? <><p className="ps49-help">AI matn, formula va rasm tavsiflarini tayyorlaydi. Natijani tekshirgandan keyin qo‘llaysiz.</p><Field label="AI uchun qo‘shimcha istaklar" value={aiInstructions} onChange={setAiInstructions} maxLength={2000} multiline rows={3} placeholder="Masalan: sodda til, kundalik hayotdan misollar, yakunda 3 ta savol." /><p className="ps49-help">Rasmlarni o‘zingiz yuklaysiz.</p></> : <p className="ps51-neutral-status" role="status">AI xizmati hozir yoqilmagan. O‘zingiz to‘ldirish va Word orqali mazmun olish ishlaydi.</p>}</div>}
          <button type="button" className="ps49-button ps49-primary ps49-continue" disabled={!!busy || aiRunning || !draft.title.trim()} onClick={() => goStep(1)}>{fillMode === 'ai' && aiAvailable ? 'Reja va AI mazmunini tayyorlash' : 'Slaydlar rejasini tayyorlash'}<ArrowRight size={18} /></button>
        </section>
        <section className="ps49-setup-side"><div className="ps49-section-heading"><LayoutTemplate size={19} /><h3>Maket oilasi</h3></div><Segments label="Tanlash usuli" value={familyMode} onChange={setFamilyMode} options={[{ value: 'auto', label: 'Avtomatik' }, { value: 'manual', label: 'O‘zim tanlayman' }]} />
          {familyMode === 'auto' ? <div className="ps51-auto-family"><span className="ps51-auto-mark"><LayoutTemplate size={22} /></span><div><strong>{TEMPLATES.find(item => item.id === recommendedFamily(draft.lesson_type))?.label || 'Klassik'}</strong><p className="ps49-help">Dars turiga mos tavsiya. Tahrir paytida istalgan oilaga almashtirasiz.</p></div><button type="button" className="ps49-text-button" onClick={() => setFamilyMode('manual')}>5 oilani ko‘rish<ChevronRight size={15} /></button><TemplateGallery value={recommendedFamily(draft.lesson_type)} templates={TEMPLATES.filter(item => item.id === recommendedFamily(draft.lesson_type))} onSelect={template => { setFamilyMode('manual'); updateDraft(previous => ({ ...previous, design: { ...previous.design, ...template.defaults, template: template.id } })); }} /></div> : <><p className="ps49-help">Har bir oilaning uchta slayd ko‘rinishini tekshiring. Klassik oilada avvalgi besh rang va ko‘rinish varianti saqlangan.</p><TemplateGallery value={draft.design.template} onSelect={template => updateDraft(previous => ({ ...previous, design: { ...previous.design, ...template.defaults, template: template.id }, slides: previous.slides.map(slide => ({ ...slide, design: null })) }))} /></>}
          <div className="ps49-start-options"><button type="button" onClick={() => { setImportOpen(true); setImportError(''); }} disabled={!!busy}><FileText size={23} /><span><strong>Word yoki tayyor matndan olish</strong><small>Avvalgi hujjat shakllari ham ishlaydi</small></span><ChevronRight size={17} /></button><button type="button" onClick={() => { if (confirmDiscard()) replaceDocument(sampleProject()); }} disabled={!!busy}><span className="ps49-option-icon">ƒ</span><span><strong>Formula namunasini ochish</strong><small>Matematika, fizika, kimyo va biologiya</small></span><ChevronRight size={17} /></button></div>
        </section></div>}
        {step === 1 && <>
