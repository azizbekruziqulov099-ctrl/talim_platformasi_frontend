...
export default function SchoolAccessCodes({ apiBase = '', token, maktabId }) {
  ...
  return <section className="space-y-5" aria-label="Shaxsiy ulanish kodlari">
    <div><h2 className="text-xl font-bold text-slate-900">Muassasaga ulanish kodini yangilash</h2>
      <p className="mt-2 text-sm text-slate-600">Oldin kiritilgan xodim, o‘quvchi yoki ota-onani toping. Uning mavjud sinfi, darslari va yuklamasi saqlanadi.</p></div>
    <form onSubmit={search} className="flex flex-wrap items-end gap-3">
      <label className="flex-1 min-w-48 text-sm font-semibold text-slate-700" htmlFor="school-code-person">Ism yoki familiya
        <input id="school-code-person" className="mt-2 block w-full rounded-xl border border-slate-300 px-4 py-3" value={query} maxLength={100} autoComplete="off" disabled={Boolean(busy)} placeholder="Masalan: Azizov"
      ...
      <button type="submit" disabled={Boolean(busy) || query.trim().length < 2} className="rounded-xl bg-teal-700 px-5 py-3 font-semibold text-white disabled:opacity-50">{busy === 'search' ? 'Qidirilmoqda…' : 'Shaxsni qidirish'}</button>
    </form>
    {searched && !people.length && <p role="status" className="text-sm text-slate-600">Bu maktabda mos shaxs topilmadi. Familiyasining bir qismini yozib ko‘ring.</p>}
    {people.length > 0 && <fieldset className="space-y-2"><legend className="mb-2 text-sm font-semibold text-slate-700">Kerakli shaxsni tanlang</legend>
      {people.map(person => <button key={person.user_id} type="button" disabled={Boolean(busy)} aria-pressed={selected?.user_id === person.user_id}
        ...
        <span className="font-semibold text-slate-900">{person.full_name}</span><span className="text-sm text-slate-500">{accessCodeRole(person)} · ID {person.user_id}</span>
      </button>)}
    </fieldset>}
    {selected && !issued && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
      ...
      <p className="mt-2 text-sm text-slate-700">Faqat shu kishining shaxsiy ulanish kodi yangilanadi. Uning oldingi ulanish kodi bekor bo‘ladi. Bu hisobning kirish parolini o‘zgartirmaydi.</p>
      <button type="button" onClick={regenerate} disabled={Boolean(busy)} className="mt-4 rounded-xl bg-teal-700 px-5 py-3 font-semibold text-white disabled:opacity-50">{busy === 'issue' ? 'Kod yaratilmoqda…' : 'Shu shaxs uchun yangi kod olish'}</button>
    </div>}
    {issued && <div className="space-y-3 rounded-2xl border border-teal-300 bg-teal-50 p-5" role="status">
      <p className="font-semibold text-slate-900">{issued.name} — yangi ulanish kodi</p>
      <p className="select-all break-all font-mono text-2xl font-bold tracking-widest text-teal-900">{issued.code}</p>
      <p className="text-sm text-slate-700">Amal qilish muddati: {issued.kod_muddati || '2 oy'}. Kodni faqat shu kishiga bering.</p>
      <p className="text-sm text-slate-700">U avval Telegram yoki Google orqali saytga kiradi, keyin Profil → Muassasaga ulanish bo‘limida shu kodni kiritadi.</p>
      <div className="flex flex-wrap gap-3"><button type="button" onClick={copy} className="rounded-xl border border-teal-700 px-4 py-2 font-semibold text-teal-800">{copied ? 'Nusxalandi' : 'Kodni nusxalash'}</button><button type="button" onClick={download} className="rounded-xl bg-teal-700 px-4 py-2 font-semibold text-white">Kod va yo‘riqnomani yuklab olish</button></div>
    </div>}
