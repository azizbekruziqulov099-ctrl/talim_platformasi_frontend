import { TRANSLATION_CONTROL_ROWS } from './interfaceTranslationMessages.js';
...
export function normalizeInterface(value) {
  ...
  return {
    translateContent: data.translateContent === true,
...
export function uzbekInterfaceCyrillic(source) {
  ...
  return String(source).replace(/[‘’ʻʼ`]/g, "'").split(/(\{[a-zA-Z][a-zA-Z0-9_]*\}|Google|Telegram|Kabutar|PDF|AI|ID)/g).map(part => {
    if (/^(\{[a-zA-Z][a-zA-Z0-9_]*\}|Google|Telegram|Kabutar|PDF|AI|ID)$/.test(part)) return part;
...
for (const row of [...TRANSLATION_ROWS, ...EXTRA_TRANSLATION_ROWS, ...LOGIN_TRANSLATION_ROWS, ...ACCOUNT_TRANSLATION_ROWS, ...CHAT_TRANSLATION_ROWS, ...TRANSLATION_CONTROL_ROWS]) {
...
export const registeredInterfaceLabels = () => [...dictionaries.keys()];
